    ...
    const UIPanel = {
        ...
        _createStyles() {
            ...
            style.textContent = `
                ...
                .esp-tag-option-check { width: 15px; color: rgba(248,250,253,0.90); font-size: 11px; }
                .esp-tag-option-label { flex: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
                .esp-tag-manual { margin-top: 8px; padding-top: 8px; border-top: 1px solid rgba(255,255,255,0.08); }
                .esp-tag-manual .esp-textarea { min-height: 54px; max-height: 92px; }
                        ...
                        <div class="esp-tag-select-wrap" id="esp-tag-select-wrap">
                            ...
                            <div class="esp-tag-menu" id="esp-tag-menu" role="listbox" aria-label="Eagle 标签列表">
                                <div class="esp-picker-head">
                                    ...
                                    <span class="esp-picker-mode">可多选</span>
                                </div>
        ...
        /** 获取当前页面和历史选择中可供快速选择的标签。 */
        getTagSuggestions() {
            let pageTags = [];
            try {
                pageTags = this._getCurrentPageTags ? this._getCurrentPageTags() : [];
            } catch (err) {
                pageTags = [];
            }
            return mergeTags(this.recentTags, pageTags, ['Fab', 'X', 'photo', 'video', 'gif', 'audio']);
        },
        ...
        renderFolderMenu() {
            ...
            const appendOptionNode = (node, depth, container, forceExpand = false) => {
                ...
                if (this.getSelectedFolderIds().includes(node.id)) {
