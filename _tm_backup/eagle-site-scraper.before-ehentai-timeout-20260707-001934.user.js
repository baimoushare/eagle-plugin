// ==UserScript==
// @name         Eagle 多网站图片批量抓取
// @namespace    eagle-site-scraper
// @version      1.0.0
// @description  批量抓取网站图片到 Eagle 素材库，支持 E-Hentai 深度扫描和 Fab.com 通用图片检测
// @author       Eagle Scraper
// @match        *://e-hentai.org/*
// @match        *://exhentai.org/*
// @match        *://www.fab.com/*
// @match        *://fab.com/*
...
(function() {
    ...
    function buildEHentaiFileName(viewerLink, actualPage, sourceUrl, mimeType = 'image/jpeg') {
        ...
        let rawBaseName = 'image';
        try {
            rawBaseName = decodeURIComponent(new URL(sourceUrl, window.location.href).pathname.split('/').pop() || 'image');
        } catch (err) {
            rawBaseName = 'image';
        }

        rawBaseName = rawBaseName.replace(/\.[^.]+$/, '');
        rawBaseName = rawBaseName.replace(/[^\w\u4e00-\u9fff.-]+/g, '_').replace(/_+/g, '_').replace(/^_+|_+$/g, '');
        ...
        return `eh_${galleryId}_p${paddedPage}_${rawBaseName}.${ext}`;
    }
     ...
     */
    async function triggerLocalDownload(url, fileName) {
    ...
    async function saveBlobLocally(blob, fileName) {
        ...
        try {
            await triggerLocalDownload(objectUrl, fileName);
        } finally {
    ...
    const EagleAPI = {
        ...
        _request(opts) {
            return new Promise((resolve, reject) => {
                ...
                const reqParams = {
                    ...
                    timeout: 5000,   // 5 秒超时（避免 Eagle 未运行时长时间阻塞）
                    onload: (resp) => {
                        try {
        ...
        async checkAlive() {
            try {
                const result = await this._request({ path: '/api/application/info' });
         ...
         * @param {string} base64Data - 纯 base64 编码的图片数据（不含 data: 前缀）
        ...
        async addFromBase64(base64Data, fileName, options = {}) {
            ...
            const payload = {
                base64: String(base64Data).replace(/^data:[^,]+,/, ''),
                name: fileName,
            ...
            if (folderIds.length > 0 && options.deferFolderBinding !== true) {
                try {
    ...
    const ImageExtractor = {
        ...
        getFabMainMediaInfo() {
            ...
            let candidates = allCandidates.filter(item => (
                ...
                item.width >= 400 &&
                item.height >= 220 &&
                !item.img.closest('button[aria-label^="Display media "]')
            ));
        ...
        /**
         * 获取 Fab 缩略图按钮列表。
         * Fab 的商品媒体缩略图按钮具备非常稳定的 aria-label：
         *   - Display media 0
         *   - Display media 1
         *   - ...
         *
        ...
        getFabThumbnailButtons() {
            return Array.from(document.querySelectorAll('button[aria-label^="Display media "]'))
                .sort((a, b) => {
                    const getIndex = (btn) => {
                        const label = btn.getAttribute('aria-label') || '';
                        const match = label.match(/Display media\s+(\d+)/i);
        ...
        getFabThumbnailStrip() {
            const firstBtn = document.querySelector('button[aria-label^="Display media "]');
    ...
    const UIPanel = {
        ...
        _createStyles() {
            ...
            style.textContent = `
                ...
                #eagle-scraper-panel.expanded .esp-body {
                    display: block;
                }
                /* ── 进度区域 ── */
                .esp-progress {
                ...
                .esp-folder-search-wrap {
                    ...
                    padding-bottom: 8px;
                    margin-bottom: 4px;
                    background: linear-gradient(180deg, rgba(20,23,30,0.96), rgba(20,23,30,0.86));
                ...
                #eagle-scraper-panel.collapsed:active .esp-collapsed-icon svg,
        ...
        _buildPanel() {
            ...
            const pageType = this.siteInfo ? this.siteInfo.pageType : '';
            const iconSvg = `
            ...
            this.container.innerHTML = `
                ...
                <div class="esp-body">
                    ...
                    <div class="esp-mode-card">
                        ...
                        <div class="esp-mode-grid" id="esp-mode-group">
                            <button class="esp-mode-btn" id="esp-mode-eagle" data-mode="eagle" type="button" aria-pressed="false">
                                <span class="esp-mode-btn-title">保存到 Eagle</span>
                                <span class="esp-mode-btn-sub">支持文件夹、标签、自动建目录</span>
                            </button>
                            <button class="esp-mode-btn" id="esp-mode-local" data-mode="local" type="button" aria-pressed="false">
                                <span class="esp-mode-btn-title">本地下载</span>
                                <span class="esp-mode-btn-sub">保存到浏览器默认下载目录</span>
                            </button>
                        </div>
                        <div class="esp-inline-note" id="esp-mode-hint">保存到 Eagle 时可设置文件夹、标签与自动建目录</div>
                    </div>
                    <div class="esp-folder esp-config-section" id="esp-eagle-folder-section">
                        <div class="esp-label">目标文件夹</div>
                        <div class="esp-folder-select-wrap" id="esp-folder-select-wrap">
                            ...
                            <div class="esp-folder-menu" id="esp-folder-menu" role="listbox" aria-label="Eagle 文件夹列表">
                                <div class="esp-folder-search-wrap">
                        ...
                        <div class="esp-inline-note" id="esp-folder-hint">可手动选择，也可在 Fab 中自动按商品名建目录</div>
                    </div>
                    <div class="esp-folder esp-config-section" id="esp-eagle-tags-section">
                        <div class="esp-label">标签</div>
                        <textarea class="esp-textarea" id="esp-tags-input" placeholder="输入标签，支持逗号或换行分隔"></textarea>
                    </div>
                    <div class="esp-switch-row esp-config-section" id="esp-auto-folder-row">
                        <label class="esp-switch-label" for="esp-auto-folder">
                            自动建目录
                            <span class="esp-switch-sub">Fab 默认按“原名｜中文名”自动创建文件夹</span>
                        </label>
                    ...
                    <button class="esp-btn esp-btn-secondary esp-btn-full" id="esp-btn-generic-scan">
                        检测并采集页面图片
                    </button>
        ...
        _bindEvents() {
            ...
            document.addEventListener('click', (e) => {
                if (!folderWrap || folderWrap.contains(e.target)) return;
                this.closeFolderMenu();
            });
            document.addEventListener('keydown', (e) => {
                if (e.key === 'Escape') this.closeFolderMenu();
            });
            ...
            // 自动建目录开关变化
            const autoFolderCheckbox = this.container.querySelector('#esp-auto-folder');
            if (autoFolderCheckbox) {
                autoFolderCheckbox.addEventListener('change', () => {
                    this.refreshFolderHint();
                });
            }
        },
        ...
        getManualTags() {
            ...
            return parseTagsInput(el ? el.value : '');
        },
        ...
        updateActionModeUI() {
            ...
            eagleOnlySelectors.forEach(selector => {
                ...
                section.classList.toggle('disabled', mode === 'local');
            ...
            // 4. 刷新模式提示文案
            const modeHint = this.container.querySelector('#esp-mode-hint');
            if (modeHint) {
                modeHint.textContent = mode === 'local'
                    ? '本地下载模式会直接保存到浏览器默认下载目录'
                    : (this.eagleAvailable ? '保存到 Eagle 时可设置文件夹、标签与自动建目录' : '请先启动 Eagle，才能保存到素材库');
            }
        ...
        getSelectedFolderName() {
            const folderId = this.container.querySelector('#esp-folder-select')?.value || '';
            if (!folderId) return '默认（根目录）';

            const found = flattenFolders(this.folders).find(folder => folder.id === folderId);
        ...
        setFolderSelection(folderId, options = {}) {
            ...
            this.syncFolderTriggerLabel();
            this.renderFolderMenu();
            if (refreshHint) this.refreshFolderHint();
        },
        ...
        renderFolderMenu() {
            ...
            /**
             * 说明：
             * 这里不再直接依赖 Eagle 返回的 children 树来渲染面板菜单，
             * 而是改为从已经验证可靠的原生 select option 列表反推目录树。
             *
             * 原因：
             * - select 已经能正确显示 657 个目录，说明它的数据一定是完整的；
             * - option 文本前缀里的全角空格已经隐含了层级深度；
             * - 基于 option 重建树，可以保证“显示层”的数据与“实际选中层”的数据完全一致。
             */
            const folderNodes = [];
            const stack = [];
            Array.from(select.options).slice(1).forEach(option => {
                const rawText = String(option.textContent || '');
                const depth = ((rawText.match(/^\u3000+/) || [''])[0] || '').length;
                const node = {
                    id: option.value,
                    label: rawText.replace(/^\u3000+/, '').trim() || '未命名文件夹',
                    depth,
                    children: [],
                };
            ...
            const appendOptionNode = (node, depth, container, forceExpand = false) => {
                ...
                item.type = 'button';
                item.className = 'esp-folder-option' + (node.id === currentValue ? ' active' : '');
                ...
                if (node.id === currentValue) {
                    ...
                    check.className = 'esp-folder-option-check';
                    check.textContent = '✓';
                    item.appendChild(check);
                }

                item.addEventListener('click', () => {
                    this.setFolderSelection(node.id);
                    this.closeFolderMenu();
                });
            ...
            rootItem.className = 'esp-folder-option' + (currentValue === '' ? ' active' : '');
            const rootSpacer = document.createElement('span');
            rootSpacer.className = 'esp-folder-option-spacer';
            ...
            if (currentValue === '') {
                const rootCheck = document.createElement('span');
                rootCheck.className = 'esp-folder-option-check';
                rootCheck.textContent = '✓';
                rootItem.appendChild(rootCheck);
            }
         ...
         */
        refreshFolderHint() {
            const hintEl = this.container.querySelector('#esp-folder-hint');
            if (!hintEl) return;

            if (this.isLocalMode()) {
                hintEl.textContent = '本地下载模式不会写入 Eagle 文件夹，文件将进入浏览器默认下载目录';
                return;
            }
            ...
            if (this.isFabContext()) {
                ...
                if (this.isAutoFolderEnabled()) {
                    hintEl.textContent = `采集时将自动创建 / 复用：${folderName}`;
                } else {
                    hintEl.textContent = `当前手动目录：${this.getSelectedFolderName()}`;
                }
                return;
            }

            hintEl.textContent = `当前目录：${this.getSelectedFolderName()}`;
        },
        ...
        buildScanTags(baseTags = []) {
            return mergeTags(baseTags, this.getManualTags());
        },
        ...
        async outputImage(options = {}) {
            ...
            if (mode === 'local') {
                ...
                await saveBlobLocally(localBlob, fileName);
                return;
            }
        ...
        async ensureFabTargetFolder() {
            ...
            const manuallySelected = this.container.querySelector('#esp-folder-select')?.value || '';
            if (manuallySelected && !this.isAutoFolderEnabled()) {
            ...
            let found = flatFolders.find(folder => folder.name === folderName);

            if (!found) {
                this.setStatus(`正在创建目录：${folderName}`);
                const created = await EagleAPI.createFolder(folderName, 'Fab 商品自动采集目录');
                found = {
                    id: created.id,
                    name: created.name,
                    depth: 0,
            ...
            this.setFolderSelection(found.id);
            return found.id;
        },
        ...
        async loadFolders() {
            ...
            if (!isAlive) {
                ...
                this.updateActionModeUI();
                this.setStatus(
                    this.isLocalMode() ? '⬇️ 本地下载模式已就绪（Eagle 未运行）' : '⚠️ Eagle 未运行',
            ...
            this.folders = await EagleAPI.getFolders();
            this.folderExpandedIds = new Set(['']);
            const select = this.container.querySelector('#esp-folder-select');
            const flatFolders = flattenFolders(this.folders);
            ...
            if (!preferredFolderId) {
                ...
                if (growthFolder) preferredFolderId = growthFolder.id;
            }

            if (preferredFolderId) {
                const exists = flatFolders.some(folder => folder.id === preferredFolderId);
                if (exists) {
                    this.setFolderSelection(preferredFolderId, {
                        persist: false,
                        refreshHint: false,
                    });
                }
            ...
            this.updateActionModeUI();
            this.setStatus(
                this.isLocalMode() ? '⬇️ 本地下载模式已就绪（Eagle 已连接）' : '✅ Eagle 已连接'
            );
            return true;
        },
    ...
    async function main() {
        ...
        // ── 步骤 4：异步检查 Eagle（不阻塞面板显示）──
        EagleAPI.checkAlive().then(alive => {
            UIPanel.eagleAvailable = !!alive;
            UIPanel.updateActionModeUI();
            if (alive) {
                Log.info('✅ Eagle 已连接');
            } else {
                Log.warn('⚠️ Eagle 未运行，面板仍会显示（采集时会报错）');
            }
        });
    ...
    function adjustUIForPageType(siteInfo) {
        ...
        const folderHint = UIPanel.container.querySelector('#esp-folder-hint');
        ...
        if (pageType === 'listing_detail' || pageType === 'any_page') {
            ...
            if (genericBtn) {
                ...
                genericBtn.textContent = '⬇️ 批量采集商品图片';
            }
            if (autoFolderRow) autoFolderRow.style.display = 'flex';
            if (folderHint) UIPanel.refreshFolderHint();
        }
    }
