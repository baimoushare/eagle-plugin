...
const TMD = (function () {
    ...
    return {
        ...
        resolveTweetSource: async function (json, status_id, options = {}) {
            ...
            const tweet = choice === 'quoted' ? quotedTweet : originalTweet
            const user = choice === 'quoted' ? quotedUser : originalUser
...
`
            ...
            /**
             * 后台时不再继续滚动时间线：X 的懒加载依赖前台渲染，强行后台滚动只会空转。
             * 已发现的推文仍可通过后台 GM 请求完成解析并写入 Eagle；回到 X 标签后会自动继续加载。
             */
            ...
            isTransientTaskError: function (err) {
                ...
                return /BACKGROUND_REQUEST|ABORT|TIMEOUT|NETWORK|FAILED TO FETCH|LOAD FAILED|ECONNRESET|ECONNREFUSED|X_GRAPHQL_EMPTY_RESULT/.test(text)
            },
            ...
            async start(ctx, mode) {
                ...
                this.setText(
                    ...
                    mode === 'download' && useLocalDirectoryWriter ? (lang.batch_writing_local || '????????????????...') : lang.batch_scanning
                )
                ...
                while (this.running && (continuousMode || idleRounds < 30)) {
                    ...
                    const ids = this.getTimelineStatusIds(ctx)
                    ...
                    if (pending.length === 0) {
                        ...
                        if (scrollResult.deferred) {
                            this.setText(
                                mode === 'download' ? lang.batch_download_running : lang.batch_eagle_running,
                                this.formatProgress('后台处理中；返回 X 页面后继续加载更多')
                            )
                            // 后台不空转；切回 X 标签后 visibilitychange 会唤醒下一轮。
...
`)
        ...
        /**
         * ???? X ??????? GraphQL operation id?
         *
         * ???TweetResultByRestId ? doc_id / queryId ?? X ??????
         * ????? ID ???? X ?????????????????
         * ???????? tweet ? fail?media ??? 0?
         *
         * ?????
         * 1. ???????????????? JS?
         * 2. ? performance resource ? script ???????????? JS?
         * 3. ?? JS ?????????? TweetResultByRestId ?? ID?
         * 4. ????????? ID?????????????
         */
        ...
        fetchJson: async function (status_id) {
            const operationId = await this.getGraphQLOperationId('TweetResultByRestId', '2ICDjqPd81tulZcYrtpTuQ')
            let base_url = `https://${host}/i/api/graphql/${operationId}/TweetResultByRestId`
            let variables = {
                'tweetId': status_id,
            ...
            let tweet_detail
            try {
                // 批量模式优先走扩展后台通道，避免切换标签页后请求被页面节流卡住。
                tweet_detail = await this.requestJsonInBackground(url, headers)
            } catch (backgroundErr) {
                // 兼容少数浏览器/脚本管理器没有把登录 Cookie 带入 GM 请求的情况：
                // 回退到原页面 fetch，但加 AbortController 防止一次请求无限挂起。
