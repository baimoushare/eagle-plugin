...
const TMD = (function () {
    let lang, host, history, show_sensitive, is_tweetdeck, eagle_root_segments, eagle_child_template, eagle_selected_folder_id
    return {
        /**
         * 统一解析当前界面应使用的语言。
         *
        ...
        prepareMediaPayload: async function (status_id, options = {}) {
            ...
            const baseTags = ['X'].filter(Boolean)
        ...
        buildImportKeys: function (task) {
            ...
            addItemByUrl: async function (task) {
                ...
                // 首次 item/add 已提交 folders；同步二次绑定是慢路径，按任务显式开启。
                if (Array.isArray(normalizedTask.folders) && normalizedTask.folders.length > 0 && task.verifyFolders === true) {
                    try {
...
<div class="tmd-batch-folder">
  <div class="tmd-batch-folder-label">Eagle &#x6587;&#x4EF6;&#x5939;</div>
  <input class="tmd-batch-folder-search" data-action="folder-search" placeholder="&#x641C;&#x7D22;&#x6587;&#x4EF6;&#x5939;..." />
  <select class="tmd-batch-folder-select" data-action="folder-select"></select>
...
`
            ...
            renderFolderOptions: function (ctx, keyword = '') {
                ...
                for (const folder of matched) {
                    ...
                    opt.textContent = `${'  '.repeat(Math.min(folder.depth || 0, 6))}${folder.name}`
        ...
        addEagleButtonToActionGroup: function (btn_group, btn_share, status_id) {
            ...
            eagleBtn.onclick = async (event) => {
                ...
                try {
                    ...
                    const payload = await this.prepareMediaPayload(status_id, { promptIfQuoted: true })
...
.tmd-batch-launcher {position: fixed; top: 18px; right: 18px; z-index: 100000; width: 52px; height: 52px; display: grid; place-items: center; padding: 0; border: 1px solid rgba(255,255,255,0.18); border-radius: 50%; background: rgba(15,23,42,0.86); color: #f8fafc; box-shadow: 0 12px 28px rgba(15,23,42,0.28), inset 0 1px 0 rgba(255,255,255,0.10); cursor: pointer; transition: transform 180ms ease, background 180ms ease, box-shadow 180ms ease;}
.tmd-batch-launcher svg {width: 26px; height: 26px; display: block;}
.tmd-batch-launcher:hover {transform: translateY(-2px) scale(1.04); background: rgba(30,41,59,0.95); box-shadow: 0 16px 32px rgba(15,23,42,0.34), inset 0 1px 0 rgba(255,255,255,0.14);}
.tmd-batch-launcher.is-open {background: rgba(37,99,235,0.92); border-color: rgba(147,197,253,0.72);}
.tmd-batch-panel {position: fixed; top: 82px; right: 18px; z-index: 99999; width: 240px; padding: 12px; border-radius: 18px; background: rgba(15, 23, 42, 0.58); color: #fff; backdrop-filter: blur(18px); -webkit-backdrop-filter: blur(18px); box-shadow: 0 12px 32px rgba(15, 23, 42, 0.22); border: 1px solid rgba(255,255,255,0.14); font-family: system-ui, -apple-system, sans-serif; opacity: 0; visibility: hidden; pointer-events: none; transform: translateY(-8px) scale(0.98); transform-origin: top right; transition: transform 180ms ease, opacity 180ms ease, visibility 180ms ease;}
.tmd-batch-panel.is-open {opacity: 1; visibility: visible; pointer-events: auto; transform: translateY(0) scale(1);}
.tmd-batch-panel:hover {transform: translateY(2px) scale(1);}
.tmd-batch-panel.running {box-shadow: 0 16px 36px rgba(15, 23, 42, 0.28);}
.tmd-batch-title {font-size: 13px; font-weight: 700; letter-spacing: 0.3px; margin-bottom: 6px;}
.tmd-batch-status, .tmd-batch-progress {font-size: 12px; line-height: 1.45; color: rgba(255,255,255,0.84);}
.tmd-batch-progress {margin-top: 2px; min-height: 18px;}
.tmd-batch-folder {margin-top: 10px; padding-top: 10px; border-top: 1px solid rgba(255,255,255,0.12);}
.tmd-batch-folder-label {font-size: 11px; color: rgba(255,255,255,0.68); margin-bottom: 6px;}
.tmd-batch-folder-search, .tmd-batch-folder-select {width: 100%; box-sizing: border-box; border: 1px solid rgba(255,255,255,0.16); border-radius: 10px; background: rgba(255,255,255,0.10); color: #fff; padding: 7px 9px; font-size: 12px; outline: none;}
.tmd-batch-folder-search::placeholder {color: rgba(255,255,255,0.48);}
.tmd-batch-folder-select {margin-top: 6px; cursor: pointer;}
.tmd-batch-folder-select option {background: #111827; color: #f8fafc;}
.tmd-batch-folder-refresh {margin-top: 6px; border: 0; background: transparent; color: rgba(255,255,255,0.72); font-size: 11px; cursor: pointer; padding: 0;}
.tmd-batch-folder-refresh:hover {color: #fff; text-decoration: underline;}
.tmd-batch-actions {display: flex; gap: 8px; margin-top: 10px; flex-wrap: wrap;}
.tmd-batch-btn {appearance: none; border: 1px solid rgba(255,255,255,0.16); background: rgba(255,255,255,0.08); color: #fff; border-radius: 999px; padding: 7px 10px; font-size: 12px; cursor: pointer; transition: transform 0.15s ease, background 0.15s ease, opacity 0.15s ease;}
.tmd-batch-btn:hover {background: rgba(255,255,255,0.14); transform: translateY(-1px);}
.tmd-batch-btn:disabled {opacity: 0.45; cursor: not-allowed; transform: none;}
.tmd-batch-btn.primary {background: rgba(37, 99, 235, 0.92); border-color: rgba(96,165,250,0.35);}
.tmd-batch-btn.primary:hover {background: rgba(37, 99, 235, 1);}
