...
const TMD = (function () {
    ...
    return {
        ...
        resolveTweetSource: async function (json, status_id, options = {}) {
            const originalTweet = json?.legacy
            const originalUser = json?.core?.user_results?.result?.legacy
            const quotedResult = json?.quoted_status_result?.result
            const quotedTweet = quotedResult?.legacy
            ...
            const effectiveStatusId = choice === 'quoted'
                ? (quotedResult?.rest_id || status_id)
                : status_id
        ...
        prepareMediaPayload: async function (status_id, options = {}) {
            ...
            const tweetUrl = `https://${host}/${user.screen_name}/status/${resolved.statusId}`
            /**
...
<div class="tmd-batch-folder">
  ...
  <div class="tmd-batch-picker" data-picker="folder">
    ...
    <div class="tmd-batch-picker-menu" data-action="folder-menu" role="listbox" aria-label="Eagle 文件夹树">
      ...
      <div class="tmd-batch-picker-footer"><span>↑↓ 移动</span><span>Enter 选择</span><span>Esc 关闭</span></div>
    </div>
  </div>
...
<div class="tmd-batch-folder tmd-batch-tags">
  ...
  <div class="tmd-batch-picker" data-picker="tags">
    ...
    <div class="tmd-batch-picker-menu tmd-batch-tags-menu" data-action="tags-menu" role="listbox" aria-label="Eagle 标签列表">
      <div class="tmd-batch-picker-head"><input class="tmd-batch-tag-search" data-action="tags-search" placeholder="&#x641C;&#x7D22;&#x6807;&#x7B7E;..." /><span>可多选</span></div>
...
`
            ...
            renderFolderOptions: function (ctx, keyword = '') {
                ...
                const appendNode = (node, depth = 0, parent = tree, forceExpand = false) => {
                    ...
                    if (String(node.id) === currentValue) {
                        const check = document.createElement('span')
                        check.className = 'tmd-batch-folder-node-check'
                        check.textContent = '✓'
                ...
                root.innerHTML = `<span class="tmd-batch-folder-toggle">&#8226;</span><span class="tmd-batch-folder-node-label">自动目录 / 根目录</span>${currentValue ? '' : '<span class="tmd-batch-folder-node-check">✓</span>'}`
                root.onclick = () => {
            ...
            togglePicker: function (kind) {
                ...
                else this.renderTagOptions(this.ctx)
            },
            ...
            getTagSuggestions: function () {
                ...
                const contextTags = path.includes('/history/likes') ? ['喜欢'] : path.includes('/bookmarks') ? ['书签'] : path.includes('/media') ? ['媒体'] : []
                return Array.from(new Set([...(this.recentTags || []), ...contextTags, 'X', 'photo', 'video', 'gif', 'audio']))
            },
            ...
            async start(ctx, mode) {
                ...
                while (this.running && (continuousMode || idleRounds < 30)) {
                    ...
                    const pending = ids.filter(id => !this.processed.has(id))
...
`)
        ...
        getGraphQLOperationId: async function (operationName, fallbackId) {
            ...
            for (const url of uniqueUrls) {
                try {
                    const source = await fetch(url, { credentials: 'include' }).then(r => r.ok ? r.text() : '')
        ...
        fetchJson: async function (status_id) {
            ...
            let tweet_detail = await fetch(url, { headers: headers }).then(async result => {
...
.tmd-batch-folder-node {display: flex; align-items: center; gap: 6px; width: 100%; box-sizing: border-box; border: 0; border-radius: 10px; padding-top: 7px; padding-bottom: 7px; padding-right: 8px; background: transparent; color: rgba(237,241,247,0.88); text-align: left; font-size: 12px; line-height: 1.35; cursor: pointer; transition: background 160ms ease, color 160ms ease, box-shadow 160ms ease, transform 140ms ease;}
...
.tmd-batch-folder-toggle {display: inline-flex; align-items: center; justify-content: center; width: 14px; min-width: 14px; color: rgba(255,255,255,0.52); font-size: 10px; transition: transform 160ms ease, color 160ms ease;}
.tmd-batch-folder-toggle.is-expanded {transform: rotate(90deg); color: rgba(255,255,255,0.88);}
.tmd-batch-folder-node-label {min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; flex: 1;}
.tmd-batch-folder-node-check {color: rgba(248,250,253,0.92); font-size: 11px; flex: 0 0 auto;}
...
.tmd-batch-picker-trigger > span:first-child {min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;}
.tmd-batch-picker-meta {flex: 0 0 auto; color: rgba(235,240,248,0.48); font-size: 10px; white-space: nowrap;}
.tmd-batch-picker-menu {position: absolute; top: calc(100% + 8px); left: auto; right: 0; z-index: 21; display: none; width: min(360px, calc(100vw - 24px)); box-sizing: border-box; padding: 8px; max-height: 420px; overflow: hidden; border: 1px solid rgba(255,255,255,0.12); border-radius: 12px; background: rgba(20,23,30,0.96); box-shadow: 0 18px 40px rgba(5,10,18,0.34), inset 0 1px 0 rgba(255,255,255,0.06); backdrop-filter: blur(18px) saturate(140%); -webkit-backdrop-filter: blur(18px) saturate(140%);}
.tmd-batch-picker.is-open .tmd-batch-picker-menu {display: block; animation: tmdPickerIn 160ms cubic-bezier(0.16,1,0.3,1);}
...
.tmd-batch-picker-footer {display: flex; justify-content: space-between; gap: 8px; padding-top: 8px; margin-top: 6px; border-top: 1px solid rgba(255,255,255,0.08); color: rgba(235,240,248,0.42); font-size: 10px;}
.tmd-batch-folder-tree {max-height: 292px; overflow-y: auto; scrollbar-width: thin;}
.tmd-batch-tag-list {display: flex; flex-direction: column; gap: 4px; max-height: 190px; overflow-y: auto; scrollbar-width: thin;}
.tmd-batch-tag-option {width: 100%; display: flex; align-items: center; gap: 8px; padding: 8px 10px; border: 0; border-radius: 9px; background: transparent; color: rgba(237,241,247,0.84); font-size: 12px; text-align: left; cursor: pointer;}
...
.tmd-batch-tag-check {width: 15px; color: rgba(248,250,253,0.90); font-size: 11px;}
...
.tmd-batch-tag-manual {width: 100%; height: 34px; margin-top: 8px; padding: 0 10px; border: 1px solid rgba(255,255,255,0.10); border-radius: 9px; background: rgba(255,255,255,0.05); color: #edf1f7; font-size: 11px; outline: none; box-sizing: border-box;}
