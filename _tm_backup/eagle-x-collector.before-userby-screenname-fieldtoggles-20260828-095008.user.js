                ...
                GM_xmlhttpRequest({
                    ...
                    onload: (response) => {
                        if (!(response.status >= 200 && response.status < 300)) {
                            reject(new Error(`X_GRAPHQL_HTTP_${response.status}: ${(response.responseText || '').slice(0, 220)}`))
                            return
                        }
...
`)
            ...
            const requestByOperationId = async id => {
                ...
                try {
                    ...
                    if (Array.isArray(data?.errors) && data.errors.length > 0) {
                        throw new Error(`X_GRAPHQL_RESPONSE_ERROR: ${JSON.stringify(data.errors).slice(0, 260)}`)
                    }
                    return data
                } catch (backgroundErr) {
        ...
        getTimelineQueryConfig: async function () {
            ...
            if (/^\/i\/history\/likes\/?$/.test(pathname)) {
                return { operationName: 'Likes', operationId: 'xA8fDIbrJfy4ojjjXmSR-A', variables: { count: 40, includePromotedContent: true } }
            }
        ...
        resolveTimelineUserId: async function (screenName) {
            ...
            const variables = { screenName: name, withSafetyModeUserFields: true, withSuperFollowsUserFields: true, withHighlightsTweets: true, withV2Timeline: true, withConnections: true }
            const features = this.buildTimelineFeatures()
            const url = encodeURI('https://' + host + '/i/api/graphql/' + operationId + '/UserByScreenName?variables=' + JSON.stringify(variables) + '&features=' + JSON.stringify(features))
        ...
        fetchTimelinePage: async function (cursor = '') {
            ...
            if (Array.isArray(data?.errors) && data.errors.length > 0) {
                throw new Error(`X_TIMELINE_GRAPHQL_ERROR: ${JSON.stringify(data.errors).slice(0, 260)} [operation=${config.operationName}; queryId=${operationId}; variables=${Object.keys(variables).join(',')}]`)
            }
