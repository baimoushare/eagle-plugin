...
(function() {
    ...
    const UIPanel = {
        ...
        localSaveAs: false,          // 目录授权不可用/被系统拒绝时，改用浏览器原生保存位置对话框
        ...
        _createStyles() {
            ...
            style.textContent = `
                ...
                .esp-mode-btn.active {
                    border: 2px solid rgba(145, 204, 255, 0.92);
                    background: linear-gradient(180deg, rgba(156, 205, 246, 0.30), rgba(82, 124, 162, 0.24)), rgba(92, 132, 168, 0.20);
                    box-shadow: inset 0 1px 0 rgba(255,255,255,0.22), 0 0 0 2px rgba(109,184,245,0.18), 0 10px 20px rgba(8,14,24,0.18);
                    transform: translateY(-1px);
                }
                ...
                .esp-btn-start {
                    ...
                    color: #ffffff;
                    border-color: rgba(255,255,255,0.16);
                    background: rgba(91, 108, 128, 0.28);
                    box-shadow:
                        inset 0 1px 0 rgba(255,255,255,0.16),
                        0 0 0 1px rgba(255,255,255,0.04),
                        0 10px 24px rgba(6, 12, 22, 0.18);
                }
                .esp-btn-start::before {
                    content: "";
                    position: absolute;
                    inset: -1px;
                    z-index: -1;
                    ...
                    background: conic-gradient(
                        ...
                        rgba(255,255,255,0.96) 292deg,
                        rgba(198,225,255,0.42) 314deg,
                        transparent 334deg 360deg
                    );
                    -webkit-mask: linear-gradient(#000 0 0) content-box, linear-gradient(#000 0 0);
                    -webkit-mask-composite: xor;
                    mask-composite: exclude;
                    animation: espStartBorderFlow 2.8s linear infinite paused;
                    pointer-events: none;
                }
                ...
                .esp-btn-start:hover::before,
                .esp-btn-start:focus-visible::before,
                .esp-btn-start:active::before {
                    animation-play-state: running;
                }
                @keyframes espStartBorderFlow {
            ...
            this.container.innerHTML = `
                ...
                <div class="esp-body">
                    ...
                    <div class="esp-mode-card">
                        <div class="esp-label">执行方式
                            <span class="esp-help" tabindex="0" data-tooltip="选择将采集结果保存到 Eagle，或直接下载到浏览器默认目录。" aria-label="执行方式说明">?</span>
                        </div>
                    ...
                    <div class="esp-folder esp-config-section" id="esp-eagle-folder-section">
                        <div class="esp-label">目标文件夹
                            <span class="esp-help" tabindex="0" data-tooltip="可手动选择目标目录；在 Fab 商品页也可按商品名自动创建或复用目录。" aria-label="目标文件夹说明">?</span>
                        </div>
                        <div class="esp-local-folder" id="esp-local-folder-section" style="display:none;">
                            ...
                            <div class="esp-local-folder-hint">未选择时使用浏览器默认下载目录</div>
                        </div>
        ...
        updateActionModeUI() {
            ...
            if (genericButton && this.isFabContext()) genericButton.textContent = mode === 'local' ? '开始下载' : '开始采集';
        ...
        /**
         * 请求用户授权一个本地下载目录。
         * 浏览器不允许脚本直接读取 Windows 的“下载”绝对路径，因此未选择时保持浏览器默认下载目录。
         */
        ...
        syncLocalDirectoryLabel() {
            ...
            if (this.localDirectoryHandle) {
                ...
                trigger.textContent = name && name !== '\\' && name !== '/' ? `已选择：${name}` : '已选择本地文件夹';
            } else if (this.localSaveAs) {
                trigger.textContent = '下载时选择保存位置';
            } else {
        ...
        async outputImage(options = {}) {
            ...
            if (mode === 'local') {
                ...
                await saveBlobLocally(localBlob, fileName, this.localDirectoryHandle, { saveAs: this.localSaveAs });
                return;
            }
    ...
    async function main() {
        ...
        EagleAPI.checkAlive().then(alive => {
            ...
            UIPanel.setEagleConnectionStatus(alive ? 'connected' : 'disconnected');
