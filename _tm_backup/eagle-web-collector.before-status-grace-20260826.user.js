    ...
    const EagleAPI = {
        ...
        async checkAlive() {
            ...
            try {
                ...
                Log.debug('Eagle 目录接口探测失败:', err?.message || err);
                return false;
            }
        },
                ...
                <div class="esp-body">
                    ...
                    <div class="esp-switch-row esp-config-section" id="esp-auto-folder-row">
                        <label class="esp-switch-label" for="esp-auto-folder">
                            自动建目录
                            <span class="esp-help" tabindex="0" data-tooltip="Fab 按商品名称，E-Hentai 按画廊标题自动创建文件夹。" aria-label="自动建目录说明">?</span>
                        </label>
        ...
        */
        ...
        async loadFolders() {
            ...
            if (!folderResult.ok) {
                ...
                this.updateActionModeUI();
                this.setStatus(
                    this.folders.length > 0 ? 'Eagle 已连接，保留现有目录' : 'Eagle 已连接，但目录暂时读取失败',
                    this.folders.length > 0 ? '' : 'error'
                );
