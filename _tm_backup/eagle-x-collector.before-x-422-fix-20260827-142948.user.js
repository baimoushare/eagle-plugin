...
`)
        ...
        getObservedTimelineRequest: function () {
            ...
            const expected = (() => {
                ...
                const parts = pathname.split('/').filter(Boolean)
                if (parts[1] === 'media') return ['UserPhotoTimeline', 'UserMedia']
        ...
        getTimelineQueryConfig: async function () {
            ...
            if (/^\/home\/?$/.test(pathname)) {
                return { operationName: 'HomeTimeline', variables: { count: 40, includePromotedContent: true, latestControlAvailable: true, requestContext: 'launch' } }
            }
            ...
            if (handle && (parts[1] === 'media' || parts[1] === 'with_replies' || parts.length === 1)) {
                ...
                const operationName = parts[1] === 'media' ? 'UserPhotoTimeline' : parts[1] === 'with_replies' ? 'UserRepliesTimeline' : 'UserOriginalsTimeline'
                const operationId = {
                    UserPhotoTimeline: 'XIHPPDFf0sv_vq-jGY2R6A',
                    UserRepliesTimeline: 'dRUXRSlEIPlVmPgOQ8Z43g',
                    UserOriginalsTimeline: 'jcbfqPu_2XMNOwVyGypRhw',
                }[operationName]
                return { operationName, operationId, variables: { userId, count: 40, includePromotedContent: true, withQuickPromoteEligibilityTweetFields: true, withVoice: true } }
            }
        ...
        fetchTimelinePage: async function (cursor = '') {
            ...
            if (config.fieldToggles) params.set('fieldToggles', JSON.stringify(config.fieldToggles))
            ...
            if (Array.isArray(data?.errors) && data.errors.length > 0) {
                throw new Error(`X_TIMELINE_GRAPHQL_ERROR: ${JSON.stringify(data.errors).slice(0, 260)}`)
            }
