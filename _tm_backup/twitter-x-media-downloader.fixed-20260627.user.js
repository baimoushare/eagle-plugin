...
const TMD = (function () {
    let lang, host, history, show_sensitive, is_tweetdeck, eagle_root_segments, eagle_child_template
    return {
        /**
         * 统一解析当前界面应使用的语言。
         *
        ...
        prepareMediaPayload: async function (status_id, options = {}) {
            ...
            const baseTags = ['X', info['user-name'], info['user-id']].filter(Boolean)
            ...
            const tasks = medias.map((media, i) => {
                ...
                return {
                     ...
                     * - 不同页面自动归档到不同目录
                     */
                    folderPath: this.buildEagleFolderPath(pageContext, info['user-id']),
                    tags: this.buildTaskTags(baseTags, info['file-type'], pageContext, info['user-id']).concat(dedupeTag ? [dedupeTag] : []),
                }
            })
        ...
        buildTaskTags: function (baseTags, fileType, pageContext, authorHandle = '') {
            const tags = new Set(Array.isArray(baseTags) ? baseTags.filter(Boolean) : [])
            if (fileType) tags.add(fileType)
            if (pageContext?.tagLabel) tags.add(pageContext.tagLabel)
            if (pageContext?.ownerHandle) {
                const normalizedOwner = String(pageContext.ownerHandle).replace(/^@/, '').trim()
                const normalizedAuthor = String(authorHandle || '').replace(/^@/, '').trim()
                if (normalizedOwner && normalizedOwner !== normalizedAuthor) {
                    tags.add(`来源页:${normalizedOwner}`)
                }
            }
        ...
        buildImportKeys: function (task) {
                ...
                if (stableName || stableWebsite || statusId) {
                    keys.push(`v1::${stableName}::${stableWebsite}::${statusId}`)
                }
            ...
            existsInEagleByUrl: async function (task) {
                ...
                for (const candidate of candidates) {
                    const normalizedCandidate = this.normalizeImportUrl(candidate)
                    if (normalizedCandidate && this.eagleUrlPresenceCache[normalizedCandidate]) {
                        return true
                    }
                }
                ...
                for (const candidate of candidates) {
                    try {
                        ...
                        if (items.length > 0) {
                            ...
                            if (!matched && dedupeTag) {
                                /**
                                 * 如果 Eagle 已按同一个 URL 返回了素材，但还没有我们的去重标签，
                                 * 说明这大概率是老数据。此处仍允许标记为“已存在”，这样下次不会重复导入。
                                 */
                                items.forEach(item => this.markKnownEagleUrl(item?.url || candidate))
                                this.markKnownEagleUrl(candidate)
                                return true
                            }
                            if (matched) {
                                items.forEach(item => this.markKnownEagleUrl(item?.url || candidate))
            ...
            markTaskImported: async function (task) {
                ...
                this.buildEagleLookupUrlCandidates(task).forEach(url => this.markKnownEagleUrl(url))
            ...
            ensureFolderPath: async function (parts) {
            ...
            normalizeTask: async function (task) {
                ...
                if (Array.isArray(task.folders) && task.folders.length > 0) {
                    ...
                    normalized.folders = await this.ensureFolderPath(task.folderPath)
                }
            ...
            addItemByUrl: async function (task) {
                ...
                const response = await this.request('/api/v2/item/add', 'POST', await this.normalizeTask(task))
                await this.markTaskImported(task)
...
`
            ...
            formatProgress: function (extra = '') {
                const base = `${lang.batch_progress}: ?? ${this.successTweets}/${this.processed.size}??? ${this.successMedia}??? ${this.skippedMedia}??? ${this.failedTweets}`
            ...
            scrollForMore: async function (ctx) {
                ...
                const forwardStep = Math.max(window.innerHeight * 1.4, 1400)
                const firstTarget = Math.min(before.top + forwardStep, Math.max(before.height - window.innerHeight, 0))
                window.scrollTo({ top: firstTarget, behavior: 'auto' })
                await ctx.sleep(900)

                const latestScroller = document.scrollingElement || document.documentElement || document.body
                const bottomTarget = latestScroller ? latestScroller.scrollHeight : document.body.scrollHeight
                window.scrollTo({ top: bottomTarget, behavior: 'auto' })
                ...
                return {
                    before,
                    after,
                    changed: after.height > before.height || after.articleCount > before.articleCount || after.top > before.top + 8,
                }
            },
            async start(ctx, mode) {
                ...
                while (this.running && idleRounds < 10) {
                    /**
                     * ????????? DOM???????????????/?????????????????
                     * ??????????????????????????????????
                     */
                    ...
                    if (pending.length === 0) {
                        ...
                        this.setText(
                            mode === 'download' ? lang.batch_download_running : lang.batch_eagle_running,
                            this.formatProgress(`${lang.batch_scrolling}?${idleRounds}/10?`)
                        )
                        ...
                        try {
                            ...
                            if (mode === 'download') {
                                ...
                                this.setText(
                                    ...
                                    savedCount > 0
                                        ? `${lang.batch_processing} ${this.processed.size}??? ${savedCount}??? ${skippedCount}`
                                        : `${lang.batch_processing} ${this.processed.size}????????? ${skippedCount}`
                                )
                                await ctx.sleep(220)
                            }
                            ...
                            this.setText(
                                mode === 'download' ? lang.batch_download_running : lang.batch_eagle_running,
                                `?? ${this.failedTweets}?${errText}`
                            )
                        }

                        this.setText(
                            mode === 'download' ? lang.batch_download_running : lang.batch_eagle_running,
                            this.formatProgress(this.lastError ? '?????' + this.lastError : '')
                        )
                        await ctx.sleep(500)
                    }
                }
                ...
                this.setText(
                    finishedByUser
                        ? lang.batch_stopped
                        : (idleRounds >= 10 && this.failedTweets === 0 ? (lang.batch_done || '???') : lang.batch_done),
                    this.formatProgress()
                )
            },
            stop: function () {
