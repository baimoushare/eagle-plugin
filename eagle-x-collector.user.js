...
// @name:zh            X、Twitter图片批量保存到Eagle
// @name:zh-CN         X、Twitter图片批量保存到Eagle
...
// @description        Download videos/pictures/audio with one click | Save to Eagle or package batch downloads
...
// @description:zh-CN  一键采集视频/图片/音频，可保存到 Eagle，或批量打包为 ZIP 下载
...
const TMD = (function () {
    let lang, host, history, show_sensitive, is_tweetdeck, eagle_root_segments, eagle_child_template, eagle_selected_folder_id, eagle_selected_tags = [], eagle_recent_tags = []
    return {
        /**
         * 统一解析当前界面应使用的语言。
         *
         * 旧逻辑只看 <html lang>：
         * - 但 X 页面经常仍然返回 en
         * - 即使用户浏览器、系统、Tampermonkey 都是中文，按钮仍会显示英文
         *
         * 这里改为：
         * 1. 优先读取页面 lang
         * 2. 再读取浏览器语言 navigator.language / navigator.languages
         * 3. 将 zh-CN / zh-SG / zh-MY 归一为 zh
         * 4. 将 zh-TW / zh-HK / zh-MO 归一为 zh-Hant
         * 5. 如果都识别不到，再回退英文
         */
        resolveLanguage: function () {
            ...
            // 第一轮：优先命中中文变体，避免 html lang= en 抢先把界面固定成英文。
            for (const raw of candidates) {
                const code = raw.toLowerCase()
                if (code.startsWith('zh-cn') || code.startsWith('zh-sg') || code.startsWith('zh-my') || code === 'zh') {
            ...
            for (const raw of candidates) {
                const code = raw.toLowerCase()
                if (this.language[raw]) return this.language[raw]
                if (this.language[code]) return this.language[code]
            }

            return this.language.en
        },
        /**
         * 把用户填写的 Eagle 根目录文本解析成稳定的目录数组。
         *
         * 输入示例：
         * - "X"
         * - "社媒采集/X"
         ...
         * - ["社媒采集", "X"]
         */
        parseEagleRootPath: function (rawValue) {
            const parts = String(rawValue || 'X')
                .split(/[\\/]+/g)
                .map(part => this.sanitizeFolderSegment(part))
                .filter(Boolean)

            return parts.length > 0 ? parts : ['X']
        },
        /**
         * 供设置面板 / 导入逻辑共用的根目录展示文本。
         */
        getEagleRootPathText: function () {
            const parts = Array.isArray(eagle_root_segments) && eagle_root_segments.length > 0
                ? eagle_root_segments
                : ['X']
            return parts.join('/')
        },
         ...
         *
         * 默认值：
         * - {page-scope}/{page-type}
         *
         * 可用占位符：
         * - {page-scope} 当前页面主体，例如 Laobai_Art / 书签 / 首页时间线
         * - {page-type} 当前页面类别，例如 喜欢 / 媒体 / 主页 / 单条推文
         * - {page-owner} 当前页面归属用户，例如 NASA / Laobai_Art
         * - {author} 当前资源作者（推文作者）
         */
        parseEagleChildTemplate: function (rawValue) {
            const template = String(rawValue || '{page-scope}/{page-type}')
                .trim()
                .replace(/\\/g, '/')
            return template || '{page-scope}/{page-type}'
        },
        /**
         * 供设置面板展示当前子目录模板。
         */
        getEagleChildTemplateText: function () {
            return this.parseEagleChildTemplate(eagle_child_template || '{page-scope}/{page-type}')
        },
        /**
         * ? Eagle ??????? select ??????
         * depth ???????path ??????????????????????
         */
        flattenEagleFolders: function (folders = [], depth = 0, path = []) {
            const out = []
            if (!Array.isArray(folders)) return out
            for (const folder of folders) {
                if (!folder || !folder.id) continue
                const name = String(folder.name || '').trim() || '\u672a\u547d\u540d\u6587\u4ef6\u5939'
                ...
                if (Array.isArray(folder.children) && folder.children.length > 0) {
                    out.push(...this.flattenEagleFolders(folder.children, depth + 1, nextPath))
                }
            }
            return out
        },
         ...
         * - ???????????????????????????????????
         */
        buildEagleFolderSpec: function (pageContext, fallbackHandle = '') {
            const selectedParentId = String(eagle_selected_folder_id || '').trim()
            if (selectedParentId) {
                const childSegments = this.resolveEagleChildTemplateSegments(pageContext, fallbackHandle)
                return {
                    folderParentId: selectedParentId,
                    folderPath: childSegments.length > 0 ? childSegments : [],
                }
            }
            return {
                folderParentId: '',
                folderPath: this.buildEagleFolderPath(pageContext, fallbackHandle),
            }
        },
        init: async function () {
            const menuLang = this.resolveLanguage()
            GM_registerMenuCommand(menuLang.settings, this.settings)
            GM_registerMenuCommand(menuLang.export_history || 'Export History (Markdown)', async () => this.exportHistory())
             ...
             * - 灵感库/X
             *
             * 最终都会被解析成目录数组，后续拼接到站点上下文目录前面。
             */
            eagle_root_segments = this.parseEagleRootPath(await GM_getValue('eagle_root_path', 'X'))
            /**
             * 读取“自动子目录模板”配置。
             * 目录模板单独拆出来后，用户就能保留一个固定父目录，
             * 同时灵活定义后续层级规则。
             */
            eagle_child_template = this.parseEagleChildTemplate(await GM_getValue('eagle_child_template', '{page-scope}/{page-type}'))
            // ????????????? Eagle ?????????????Eagle ?????????
             eagle_selected_folder_id = String(await GM_getValue('eagle_selected_folder_id', '') || '')
            const savedSelectedTags = await GM_getValue('eagle_selected_tags', [])
            const savedRecentTags = await GM_getValue('eagle_recent_tags', [])
            eagle_selected_tags = Array.isArray(savedSelectedTags) ? savedSelectedTags.filter(Boolean).slice(0, 24) : []
            eagle_recent_tags = Array.isArray(savedRecentTags) ? savedRecentTags.filter(Boolean).slice(0, 24) : []
            window.__TMD_LANG_DEBUG = {
                htmlLang: document.querySelector('html')?.lang || '',
                navigatorLanguage: navigator.language || '',
                resolvedDownloadText: lang && lang.download,
                resolvedSettingsText: lang && lang.settings,
            }
            /**
             * 暴露一个只读调试入口，便于后续排查用户环境问题。
             * 这不会影响普通用户使用，但在后台自动化测试里很有帮助。
             */
            window.__TMD_DEBUG__ = this
            ...
            /**
             * 这里必须 await：
             * GM_getValue 在新版 Tampermonkey 中可能返回 Promise。
             * 如果不等待，show_sensitive 会变成 Promise 对象，
             * 后续 if (show_sensitive) 判断会始终为真，带来一堆误判。
             */
            show_sensitive = await GM_getValue('show_sensitive', false)
            ...
            /**
             * 关键修复 1：
             * 原脚本只监听“后续新增节点”，没有扫描“页面初始已存在的推文内容”。
             * 在 X 这种 SPA 页面里，这会直接导致页面已经渲染完成时完全不显示下载按钮。
             *
             * 因此这里补 3 层兜底：
             * 1. 初始化立即全页扫描
             * 2. 延迟再次扫描，兼容页面分阶段渲染
             * 3. 定时轻量重扫，兼容 X 的无刷新路由切换 / 懒加载
             */
            this.scanExisting(document)
            setTimeout(() => this.scanExisting(document), 1200)
            setTimeout(() => this.scanExisting(document), 3000)
            setInterval(() => this.scanExisting(document), 2500)
            this.ensureBatchPanel()
        },
        ...
        /**
         * 主动扫描当前页面中已经存在的推文与媒体列表项。
         * 这是给 X/Twitter 单页应用补上的“首屏扫描 + 路由切换重扫”能力。
         */
        scanExisting: function (root) {
            let scope = root || document
            scope.querySelectorAll && scope.querySelectorAll('article').forEach(article => this.addButtonTo(article))
            scope.querySelectorAll && scope.querySelectorAll('li[role="listitem"]').forEach(li => this.addButtonToMedia([li]))
        },
        /**
         * 从推文节点中安全提取 status id。
         * X 页面结构经常变化，不能再直接假设 querySelector 一定有值。
         */
        getStatusId: function (root, fallback) {
            if (fallback) return fallback
            let link = root && root.querySelector && root.querySelector('a[href*="/status/"]')
            if (!link || !link.href) return ''
            let match = link.href.match(/\/status\/(\d+)/)
            return match ? match[1] : ''
        },
        /**
         * 找到操作按钮组。
         * 旧脚本只用 last-of-type，过于依赖结构顺序；这里做多一层兼容。
         */
        getActionGroup: function (article) {
            const groups = Array.from(article.querySelectorAll(
                'div[role="group"], ul.tweet-actions, ul.tweet-detail-actions'
            ))
            // X 的 DOM 会在不同页面/版本中生成多个操作组；优先选择包含分享按钮或普通按钮的最后一组。
            const mediaGroup = groups.filter(group =>
                group.querySelector(
                    'button[data-testid="share"], button[aria-label*="Share"], button[aria-label*="分享"], button'
                )
            )
            return mediaGroup.pop() || groups.pop() || null
        },
        /**
         * 找到“分享按钮”的外层容器，用于 clone 出下载按钮。
         * 若匹配不到，则返回按钮组最后一个子项作为兜底。
         */
        getShareItem: function (btn_group) {
            if (!btn_group) return null
            let candidates = Array.from(btn_group.querySelectorAll(':scope>div>div, li.tweet-action-item>a, li.tweet-detail-action-item>a'))
            let last = candidates.pop()
            if (last && last.parentNode) return last.parentNode

            // 新版 X 不一定保留旧的 div>a 层级，改用语义属性寻找分享按钮。
            const shareButton = btn_group.querySelector(
                'button[data-testid="share"], button[aria-label*="Share"], button[aria-label*="分享"]'
            )
            if (shareButton) {
                return shareButton.closest('div[role="button"]')
                    || shareButton.parentElement
                    || shareButton
            }

            return btn_group.lastElementChild || null
        },
        addButtonTo: function (article) {
            ...
            let media_selector = [
                ...
                '[aria-label*="audio" i]',
                '[aria-label*="语音"]',
                'div[aria-labelledby]>div:first-child>div[role="button"][tabindex="0"]' // X 旧版音频播放器
            ]
            ...
            let current_tweet_id = document.location.href.includes('/status/') 
                ...
                : undefined
            if (media) {
                if (article.querySelector('.tmd-down:not(.tmd-img):not(.tmd-media)')) return
                let status_id = this.getStatusId(article, current_tweet_id)
                if (!status_id) return
                let btn_group = this.getActionGroup(article)
                let btn_share = this.getShareItem(btn_group)
                if (!btn_group || !btn_share) return
                let btn_down = btn_share.cloneNode(true)
                let button = btn_down.querySelector('button')
                ...
                this.addEagleButtonToActionGroup(btn_group, btn_share, status_id)
                if (show_sensitive) {
            ...
            if (imgs.length > 1) {
                ...
                if (!status_id) return
                imgs.forEach(img => {
                    if (!img.parentNode || img.parentNode.querySelector('.tmd-down.tmd-img')) return
        ...
        addButtonToMedia: function (listitems) {
            listitems.forEach(li => {
                if (!li || li.querySelector(':scope > .tmd-down.tmd-media')) return
                let status_id = this.getStatusId(li)
                if (!status_id) return
                let is_exist = history.indexOf(status_id) >= 0
                let btn_down = document.createElement('div')
                btn_down.innerHTML = '<div><div><svg viewBox="0 0 24 24" style="width: 18px; height: 18px;">' + this.svg + '</svg></div></div>'
        ...
        /**
         * 通过 Tampermonkey 后台请求读取 X 的 JSON。
         *
         * 普通 fetch 依附于当前网页标签；切到其他标签或应用后，浏览器会节流页面任务，
         * 某些情况下 X GraphQL 请求会长时间不返回。GM_xmlhttpRequest 由扩展后台发起，
         * 更适合批量保存这种允许页面退到后台继续执行的任务。
         */
        /**
         * 将 X GraphQL 的错误压缩成可诊断但不泄露凭据的信息。
         * 旧实现只截取原始响应前 220 个字符，常把 path 截断成“variab...”。
         */
        formatGraphQLErrorMessage: function (responseText, status, url = '') {
            let payload = null
            try { payload = JSON.parse(String(responseText || '{}')) } catch (err) {}
            const errors = Array.isArray(payload?.errors) ? payload.errors : []
            const details = errors.slice(0, 3).map(error => {
                const message = String(error?.message || '').trim()
                const path = Array.isArray(error?.path) ? error.path.join('.') : ''
                const code = String(error?.extensions?.code || '').trim()
                return [message, path ? 'path=' + path : '', code ? 'code=' + code : ''].filter(Boolean).join(' | ')
            }).filter(Boolean)
            let operation = ''
            try {
                const match = String(url || '').match(/\/graphql\/([^/?]+)\/([^?]+)/i)
                if (match) operation = 'operation=' + decodeURIComponent(match[2]) + '; queryId=' + match[1]
            } catch (err) {}
            const body = details.join(' || ') || String(responseText || '').replace(/\s+/g, ' ').trim().slice(0, 600)
            return ['X_GRAPHQL_HTTP_' + status, operation, body].filter(Boolean).join(': ')
        },
        formatGraphQLPayloadErrors: function (payload, url = '', prefix = 'X_GRAPHQL_RESPONSE_ERROR') {
            const errors = Array.isArray(payload?.errors) ? payload.errors : []
            const details = errors.slice(0, 3).map(error => {
                const message = String(error?.message || '').trim()
                const path = Array.isArray(error?.path) ? error.path.join('.') : ''
            ...
            return [prefix, operation, details.join(' || ') || 'GraphQL response contains errors'].filter(Boolean).join(': ')
        },
        requestTextInBackground: function (url, headers = {}, timeout = 45000) {
                ...
                GM_xmlhttpRequest({
                    ...
                    onload: (response) => {
                        if (!(response.status >= 200 && response.status < 300)) {
                            reject(new Error(this.formatGraphQLErrorMessage(response.responseText, response.status, url)))
                            return
                        }
                        resolve(String(response.responseText || ''))
                    },
                    onerror: () => reject(new Error('X_GRAPHQL_BACKGROUND_REQUEST_FAILED')),
                    ontimeout: () => reject(new Error('X_GRAPHQL_BACKGROUND_REQUEST_TIMEOUT')),
                })
            })
        },
        requestJsonInBackground: async function (url, headers = {}, timeout = 45000) {
            const text = await this.requestTextInBackground(url, headers, timeout)
            try {
                return JSON.parse(text || '{}')
            } catch (err) {
                throw new Error('X_GRAPHQL_INVALID_JSON')
            }
        },
        /**
        ...
        localDownload: {
            ...
            saveTask: async function (task, dirHandle) {
                 ...
                 * ??????????? fetch(task.url)?
                 * - X ??? CDN ??????????????/CORS ?????
                 * - ? Tampermonkey ? GM_xmlhttpRequest ??????????? Blob?
                 *
                 * ???????????
                 * GM_xmlhttpRequest -> Blob -> File System Access API ??????????
                 */
        ...
        /**
         * 供文件名清洗和注释文本清洗共用的非法字符映射表。
         */
        invalidChars: function () {
            return { '\\': '＼', '\/': '／', '\|': '｜', '<': '＜', '>': '＞', ':': '：', '*': '＊', '?': '？', '"': '＂', '\u200b': '', '\u200c': '', '\u200d': '', '\u2060': '', '\ufeff': '', '🔞': '' }
        },
        /**
         * 判断一条推文 JSON 中是否真的存在可下载媒体。
         * @param {object} tweet
         * @returns {boolean}
         */
        /**
         ...
         * 2. entities.media????????? entities
         * 3. quoted / legacy ????? resolveTweetSource ??
         */
        extractTweetMedia: function (tweet) {
            ...
            const media = candidates.find(item => Array.isArray(item) && item.length > 0)
            const out = Array.isArray(media) ? media.filter(Boolean) : []

            // X 的语音/音频推文在不同 GraphQL 版本中字段名不稳定：
            // 可能挂在 voice_info、audio_info，或直接作为 media.type=audio 返回。
            // 这里统一转成 media 任务，后续下载和 Eagle 保存共用同一条链路。
            const audioInfo = tweet?.voice_info || tweet?.audio_info
            if (audioInfo && Array.isArray(audioInfo.variants) && audioInfo.variants.length > 0) {
                out.push({
                    type: 'audio',
                    id_str: audioInfo.media_id_string || audioInfo.media_id || '',
                    media_key: audioInfo.media_key || '',
                    audio_info: audioInfo,
                })
            }
            return out
        },
        getMediaDownloadInfo: function (media) {
            if (!media) return null
            if (media.type === 'photo' && media.media_url_https) {
                return { url: media.media_url_https + ':orig', fileType: 'photo' }
            }

            const directAudioUrl = media.audio_info?.url || media.voice_info?.url || media.audio_url
            if (directAudioUrl) {
                return {
                    url: directAudioUrl,
                    fileType: 'audio',
                    contentType: String(media.audio_info?.content_type || media.voice_info?.content_type || 'audio/mp4').toLowerCase(),
                }
            }

            const info = media.audio_info || media.voice_info || media.video_info || media.ext_tw_video_info || media
            const variants = Array.isArray(info?.variants) ? info.variants : []
            const hasAudioVariant = variants.some(item => {
                const type = String(item?.content_type || '').toLowerCase()
                return type.startsWith('audio/') || /\.(?:m4a|mp3|aac|wav)(?:[?#]|$)/i.test(String(item?.url || ''))
            })
            const isAudio = media.type === 'audio' || media.audio_info || media.voice_info || hasAudioVariant
            const usable = variants
                .filter(item => item && item.url)
                .filter(item => {
                    const type = String(item.content_type || '').toLowerCase()
                    return isAudio
                        ? type.startsWith('audio/') || type === 'video/mp4' || /\.(?:m4a|mp3|aac|wav)(?:[?#]|$)/i.test(item.url)
            ...
            const type = String(chosen.content_type || '').toLowerCase()
            const fileType = isAudio ? 'audio' : (media.type === 'animated_gif' ? 'gif' : 'video')
            return { url: chosen.url, fileType, contentType: type }
        },
        hasDownloadableMedia: function (tweet) {
            return this.extractTweetMedia(tweet).length > 0
        },
        /**
         * 兼容 X 新版 GraphQL 的多层包装：
         * TweetWithVisibilityResults / TweetTombstone / result.tweet / result.result。
         * 只要节点本身带 legacy、core 或 rest_id，就视为可继续处理的推文节点。
         */
        unwrapTweetResult: function (node, depth = 0) {
            if (!node || typeof node !== 'object' || depth > 8) return null
            if (node.legacy || node.rest_id || node.note_tweet) return node
            const candidates = [node.tweet, node.result, node.data, node.itemContent?.tweet_results?.result]
            for (const candidate of candidates) {
                const unwrapped = this.unwrapTweetResult(candidate, depth + 1)
                if (unwrapped) return unwrapped
            }
            return null
        },
        /** 从新版/旧版用户节点中统一提取 legacy 用户信息。 */
        extractTweetUser: function (node) {
            const candidates = [
            ...
            for (const candidate of candidates) {
                const user = candidate?.legacy || candidate
                if (user && (user.screen_name || user.name)) return user
            }
            return null
        },
        /**
         * 批量遍历期间持续收割页面 DOM 里的规范推文链接，形成 statusId -> 规范 URL 缓存。
         *
         * 背景：X 的 Likes 等时间线 GraphQL 返回的推文节点普遍缺少 core.user_results，
         * 拿不到作者 screen_name，导致素材来源 URL 退化成 x.com/i/web/status/<id>；
         * 而 Eagle 的 url= 查询是精确匹配，与库里按规范格式入库的存量互相查不到。
         * 时间线虚拟滚动会把旧锚点卸载，所以必须在推文经过视口时就把链接记下来。
         */
        statusUrlHints: new Map(),
        harvestStatusUrlHints: function () {
            try {
                document.querySelectorAll('a[href*="/status/"]').forEach(anchor => {
                    const href = String(anchor.href || '').trim()
                    // 只收割带真实用户名的规范链接，排除 x.com/i/web/... 与 x.com/i/status/...；
                    // 重组出干净 URL，去掉 /photo/1、/analytics 之类的路径尾巴。
                    const match = href.match(/^(https?:\/\/[^/]+)\/(?!i(?:\/|$))([^/]+)\/status\/(\d+)/)
                    if (match && !this.statusUrlHints.has(match[3])) {
                        this.statusUrlHints.set(match[3], `${match[1]}/${match[2]}/status/${match[3]}`)
                    }
                })
            } catch (err) {}
        },
        /** 只从当前页面 DOM 取 status 对应的真实 URL，供 Eagle 去重兜底。 */
        getStatusUrlCandidates: function (statusId) {
            const id = String(statusId || '').trim()
            if (!id) return []
            const urls = []
            // 先收割当前 DOM 并查缓存：推文刚滚过视口时锚点已被记下，
            // 即使此刻虚拟滚动已把它卸载，也能取回规范 URL，避免退化成 i/web 格式。
            this.harvestStatusUrlHints()
            const hinted = this.statusUrlHints.get(id)
            if (hinted) urls.push(hinted)
            try {
                document.querySelectorAll('a[href*="/status/"]').forEach(anchor => {
                    const href = String(anchor.href || '').trim()
                    const match = href.match(/\/status\/(\d+)/)
                    if (match && match[1] === id) urls.push(href.split('#')[0])
                })
            } catch (err) {}
            urls.push('https://' + host + '/i/web/status/' + id)
            urls.push('https://' + host + '/i/status/' + id)
            return Array.from(new Set(urls))
        },
        /**
         * 解析当前应使用“原始推文”还是“引用推文”的媒体来源。
         *
         * 单张点击时：
         * - 如果存在引用媒体，仍然弹出选择框
         *
         ...
         * - 若原始推文没媒体，则自动退回引用推文
         */
        resolveTweetSource: async function (json, status_id, options = {}) {
            const originalNode = this.unwrapTweetResult(json) || json
            const originalTweet = originalNode?.legacy || originalNode
            const originalUser = this.extractTweetUser(originalNode) || this.extractTweetUser(json)
            const quotedResult = originalNode?.quoted_status_result?.result || json?.quoted_status_result?.result
            const quotedNode = this.unwrapTweetResult(quotedResult)
            const quotedTweet = quotedNode?.legacy || quotedNode
            const quotedUser = this.extractTweetUser(quotedNode) || this.extractTweetUser(quotedResult)
            const retweetedResult = originalTweet?.retweeted_status_result?.result || originalNode?.retweeted_status_result?.result
            const retweetedNode = this.unwrapTweetResult(retweetedResult)
            const retweetedTweet = retweetedNode?.legacy || retweetedNode
            const retweetedUser = this.extractTweetUser(retweetedNode) || this.extractTweetUser(retweetedResult)

            const originalHasMedia = this.hasDownloadableMedia(originalTweet)
            const quotedHasMedia = this.hasDownloadableMedia(quotedTweet)
            const retweetedHasMedia = this.hasDownloadableMedia(retweetedTweet)

            let choice = 'original'

            if (options.preferredChoice === 'quoted' && quotedHasMedia) {
                choice = 'quoted'
            } else if (options.preferredChoice === 'original' && originalHasMedia) {
                choice = 'original'
            } else if (quotedHasMedia && originalHasMedia && options.promptIfQuoted !== false) {
                const originalLabel = `${originalUser?.name || ''} @${originalUser?.screen_name || ''}`.trim()
                const quotedLabel = `${quotedUser?.name || ''} @${quotedUser?.screen_name || ''}`.trim()
                const selected = await this.selectTweetDialog(originalLabel, quotedLabel)
                if (!selected) throw new Error('CANCELLED')
                choice = selected
            } else if (!originalHasMedia && retweetedHasMedia) {
                choice = 'retweeted'
            } else if (!originalHasMedia && quotedHasMedia) {
                choice = 'quoted'
            } else {
                choice = 'original'
            }

            const tweet = choice === 'quoted' ? quotedTweet : choice === 'retweeted' ? retweetedTweet : originalTweet
            const user = choice === 'quoted' ? quotedUser : choice === 'retweeted' ? retweetedUser : originalUser
            const effectiveStatusId = choice === 'quoted'
                ? (quotedNode?.rest_id || quotedResult?.rest_id || status_id)
                : choice === 'retweeted'
                    ? (retweetedNode?.rest_id || retweetedResult?.rest_id || status_id)
                    : status_id
         ...
         * - 保存到 Eagle
         */
        prepareMediaPayload: async function (status_id, options = {}) {
            const out = (await GM_getValue('filename', filename)).split('\n').join('')
            const json = options.json || await this.fetchJson(status_id)
            const resolved = await this.resolveTweetSource(json, status_id, options)
            if (!resolved) return null

            const tweet = resolved.tweet
            // X 对已删除、受限或新版包装的推文可能不返回作者 legacy；
            // 只要媒体数据仍在，就使用当前页面 URL 作为来源，不要误报 TWEET_NOT_FOUND。
            const user = resolved.user || this.extractTweetUser(json) || {
                name: 'X',
                screen_name: 'i',
            }
            const invalid_chars = this.invalidChars()

            if (json?.card && !this.hasDownloadableMedia(tweet)) {
                throw new Error('This tweet contains a link, which is not supported by this script.')
            }
            if (!tweet) {
                throw new Error('X_TWEET_UNAVAILABLE')
            }

            let medias = this.extractTweetMedia(tweet)
            if (!Array.isArray(medias) || medias.length === 0) {
                throw new Error('MEDIA_NOT_FOUND')
            }
            if (options.index) medias = [medias[options.index - 1]].filter(Boolean)

            const datetime = out.match(/\{date-time(-local)?:[^{}]+\}/)
                ? out.match(/\{date-time(?:-local)?:([^{}]+)\}/)[1].replace(/[\\/|<>*?:"]/g, v => invalid_chars[v])
                : 'YYYYMMDD-hhmmss'

            const info = {}
            info['status-id'] = resolved.statusId
            info['user-name'] = String(user.name || user.screen_name || 'X').replace(/([\\/|*?:"\u200b-\u200d\u2060\ufeff]|🔞)/g, v => invalid_chars[v])
            info['user-id'] = String(user.screen_name || 'i')
            ...
            const tweetUrl = user.screen_name && user.screen_name !== 'i'
                ? `https://${host}/${user.screen_name}/status/${resolved.statusId}`
                : (this.getStatusUrlCandidates(resolved.statusId)[0] || `https://${host}/i/web/status/${resolved.statusId}`)
            /**
             * 根据当前浏览页面，推导“这次保存动作来自哪里”。
             *
             * 这里的语义非常重要：
             * 1. 推文作者（user-id）描述的是“资源是谁发的”；
             * 2. 页面上下文（likes / media / bookmarks）描述的是“我是从哪个列表里采集的”。
             *
             * 后续保存到 Eagle 时：
             * - 标签会同时保留“作者信息”和“页面来源信息”
             * - 文件夹则优先按“当前页面上下文”分组，避免 Likes 页面里不同作者被拆散
             */
            const pageContext = this.getCurrentPageContext(user.screen_name)
            const baseTags = ['X', ...(Array.isArray(options.extraTags) ? options.extraTags : [])].filter(Boolean)

            const tasks = medias.map((media, i) => {
                const mediaInfo = this.getMediaDownloadInfo(media)
                if (!mediaInfo || !mediaInfo.url) return null
                info.url = mediaInfo.url
                /**
                 * 为每个媒体生成稳定指纹：
                 * - 优先使用 X 返回的媒体唯一 id / media_key
                 * - 如果拿不到，再退回到归一化后的原始媒体 url
                 *
                 * 这样即使你后续改了文件命名规则，
                 * 已导入过的媒体仍然可以被正确识别并跳过。
                 */
                const stableMediaId = String(media?.id_str || media?.media_key || media?.id || '').trim()
                const normalizedMediaUrl = String(
                    mediaInfo.url || ''
                )
                    .replace(/:orig$/i, '')
                    .replace(/[?#].*$/, '')
                const mediaFingerprint = [
                    String(resolved.statusId || '').trim(),
                    stableMediaId || normalizedMediaUrl,
                    String(media.type || '').trim(),
                ].filter(Boolean).join('::')
                const urlFile = info.url.split('/').pop().split(/[:?]/).shift()
                const contentTypeExt = String(mediaInfo.contentType || '').toLowerCase().split('/').pop()
                const fallbackExt = mediaInfo.fileType === 'audio'
                    ? (contentTypeExt === 'mpeg' ? 'mp3' : contentTypeExt === 'mp4' ? 'm4a' : contentTypeExt || 'm4a')
                    : mediaInfo.fileType === 'video' ? 'mp4' : mediaInfo.fileType === 'gif' ? 'gif' : 'jpg'
                // 音频 URL 有时沿用 X 的 .mp4 路径，但实际响应是 audio/mp4；
                // 统一改为 .m4a，避免 Eagle 和本地文件管理器把它误识别成视频。
                const urlExtension = urlFile.match(/\.([a-z0-9]{2,5})$/i)?.[1]?.toLowerCase()
                const resolvedExtension = mediaInfo.fileType === 'audio'
                    ? fallbackExt
                    : (urlExtension || fallbackExt)
                info.file = 'media-' + (i + 1) + '.' + resolvedExtension
                info['file-name'] = info.file.split('.').shift()
                info['file-ext'] = info.file.split('.').pop()
                info['file-type'] = mediaInfo.fileType || String(media.type || 'media').replace('animated_', '')
                info.out = (out.replace(/\.?\{file-ext\}/, '') + ((medias.length > 1 || options.index) && !out.match('{file-name}') ? '-' + (options.index ? options.index - 1 : i) : '') + '.{file-ext}')
                    .replace(/\{([^{}:]+)(:[^{}]+)?\}/g, (match, name) => info[name])

                const mediaIndex = options.index ? options.index - 1 : i
                // buildEagleDedupeTag ?? Eagle ???????? this.buildEagleDedupeTag?
                // ????????? prepareMediaPayload ???????
                // ?????????? Eagle ???? media=0??? fail?
                ...
                const taskFolderSpec = this.buildEagleFolderSpec(pageContext, info['user-id'])
                ...
                return {
                    ...
                    mediaFingerprint,
                    dedupeTag,
                    annotation: `${info['full-text']}\n${tweetUrl}\n[TMD] ${dedupeTag}`.trim(),
                    /**
                     * 注意：
                     * 这里不直接把 folder id 写死到任务里，而是先记录 folderPath。
                     * 真正发给 Eagle 之前，再通过 Eagle Web API 自动查找 / 创建目录并换成 folder id。
                     * 这样能兼容：
                     * - 首次使用时目录不存在
                     * - 不同页面自动归档到不同目录
                     */
                    folderPath: taskFolderSpec.folderPath,
                    folderParentId: taskFolderSpec.folderParentId,
                    tags: this.buildTaskTags(baseTags, info['file-type'], pageContext, info['user-id']),
                }
            }).filter(Boolean)
         ...
         *
         * 目录设计原则：
         * - 个人 Likes 页面：X / 用户名 / 喜欢
         * - 个人 Media 页面：X / 用户名 / 媒体
         * - 单条推文详情：X / 用户名 / 单条推文
         * - 书签页：X / 书签
         * - 首页时间线：X / 首页时间线
         *
         * 这样做的目的，是让“从哪里采集的”比“是谁发的”更适合作为文件夹维度；
         * 作者信息继续保留在标签里，后续搜索时不会丢。
         */
        getCurrentPageContext: function (fallbackHandle = '') {
            const pathname = String(location.pathname || '/')
            const parts = pathname.split('/').filter(Boolean)
            const reserved = new Set([
                'home', 'explore', 'search', 'notifications', 'messages', 'i', 'settings',
                'compose', 'jobs', 'communities', 'lists', 'topics', 'privacy', 'tos',
                'login', 'signup', 'intent', 'share', 'download'
            ])
            const isHandle = (value) => {
                const v = String(value || '').trim().replace(/^@/, '')
                return !!v && !reserved.has(v.toLowerCase())
            }
            ...
            if (parts[0] === 'i' && parts[1] === 'bookmarks') {
                pageType = 'bookmarks'
                folderScope = '书签'
                tagLabel = '书签'
            } else if (parts[0] === 'i' && parts[1] === 'history' && parts[2] === 'likes') {
                /**
                 * X 将“喜欢”从 /<用户名>/likes 调整为 /i/history/likes。
                 * 新路径不包含用户名，因此不再把推文作者误作为来源页归档；
                 * 统一归到 X / 喜欢，并保留“喜欢”来源标签。
                 */
                pageType = 'likes'
                folderScope = '喜欢'
                folderLabel = ''
                ...
                folderScope = '搜索'
                tagLabel = '搜索'
            } else if (parts.length >= 2 && isHandle(parts[0]) && parts[1] === 'likes') {
                ownerHandle = parts[0]
                pageType = 'likes'
                folderScope = ownerHandle
                folderLabel = '喜欢'
                tagLabel = '喜欢'
            } else if (parts.length >= 2 && isHandle(parts[0]) && parts[1] === 'media') {
                ownerHandle = parts[0]
                pageType = 'media'
                folderScope = ownerHandle
                folderLabel = '媒体'
                tagLabel = '媒体'
            } else if (parts.length >= 2 && isHandle(parts[0]) && parts[1] === 'with_replies') {
                ownerHandle = parts[0]
                pageType = 'with_replies'
                folderScope = ownerHandle
                folderLabel = '回复'
                tagLabel = '回复'
            } else if (parts.length >= 2 && isHandle(parts[0]) && parts[1] === 'status') {
                ownerHandle = parts[0]
                pageType = 'status'
                folderScope = ownerHandle
                folderLabel = '单条推文'
                tagLabel = '单条推文'
            } else if (parts.length >= 1 && isHandle(parts[0])) {
                ownerHandle = parts[0]
                pageType = 'profile'
                folderScope = ownerHandle
                folderLabel = '主页'
            ...
            /**
             * 某些场景（例如弹层详情、接口返回的推文作者明确，但 URL 不完整）拿不到页面 handle。
             * 这时仅在“缺少明确页面分组”的情况下，才退回使用推文作者作为 scope。
             */
            if (!folderScope && fallbackHandle) {
                folderScope = String(fallbackHandle || '').replace(/^@/, '').trim()
            }
            ...
            return {
                pathname,
                ownerHandle: String(ownerHandle || '').replace(/^@/, '').trim(),
                pageType,
                folderScope: this.sanitizeFolderSegment(folderScope),
                folderLabel: this.sanitizeFolderSegment(folderLabel),
                tagLabel,
            }
        },
        /**
         * 统一清洗 Eagle 目录名，避免因为 Windows / Eagle 不接受的字符导致创建失败。
         */
        sanitizeFolderSegment: function (value) {
            return String(value || '')
                .replace(/[\\/:*?"<>|\u0000-\u001f]/g, ' ')
                .replace(/\s+/g, ' ')
                .trim()
         ...
         * 将子目录模板中的占位符替换成真实值。
         *
         * 之所以单独做模板层，是为了支持这类目录策略：
         * - 固定父目录：社媒采集 / X
         * - 自动子目录：{page-scope}/{page-type}
         * - 或更细：{page-type}/{author}
         */
        resolveEagleChildTemplateSegments: function (pageContext, authorHandle = '') {
            const template = this.getEagleChildTemplateText()
            const author = this.sanitizeFolderSegment(String(authorHandle || '').replace(/^@/, '').trim())
            const tokenMap = {
                '{page-scope}': this.sanitizeFolderSegment(pageContext?.folderScope || ''),
                '{page-type}': this.sanitizeFolderSegment(pageContext?.folderLabel || ''),
                '{page-owner}': this.sanitizeFolderSegment(pageContext?.ownerHandle || ''),
                '{author}': author,
            }

            const rawSegments = String(template || '')
                .split(/[\\/]+/g)
                .map(part => {
                    ...
                    return this.sanitizeFolderSegment(next)
                })
                .filter(Boolean)

            /**
             * 去掉连续重复段，避免出现：
             * - X / Laobai_Art / Laobai_Art
             * - 书签 / 书签
             */
            const deduped = []
        ...
        /**
         * 根据页面上下文生成 Eagle 的目录路径（目录名数组）。
         * 这里只生成“名字路径”，真正的 folder id 会在 eagle.normalizeTask 里自动解析。
         */
        buildEagleFolderPath: function (pageContext, fallbackHandle = '') {
            /**
             * 目录结构规则：
             * 1. 先使用用户可配置的 Eagle 根目录
             * 2. 再拼接当前页面归档层级
             *
             * 例如：
             * - 根目录 = X
             *   => X / Laobai_Art / 喜欢
             * - 根目录 = 社媒采集/X
             *   => 社媒采集 / X / Laobai_Art / 喜欢
             */
            const segments = Array.isArray(eagle_root_segments) && eagle_root_segments.length > 0
                ? [...eagle_root_segments]
                : ['X']
            const dynamicSegments = this.resolveEagleChildTemplateSegments(pageContext, fallbackHandle)
            if (dynamicSegments.length === 0) {
                const scope = this.sanitizeFolderSegment(pageContext?.folderScope || fallbackHandle || '时间线')
                const leaf = this.sanitizeFolderSegment(pageContext?.folderLabel || '')
                if (scope) segments.push(scope)
                if (leaf && leaf !== scope) segments.push(leaf)
            } else {
        ...
        /**
         * 给 Eagle 标签补上“页面来源语义”，方便后续检索。
         * 例如：
         * - 喜欢
         * - 书签
         * - 来源页:Laobai_Art
         */
        buildTaskTags: function (baseTags, fileType, pageContext, authorHandle = '') {
            /**
             * ???????????????
             * ???X????????/??/??????????photo/video/gif??
             * ????????? ID?????tmd ?????
             */
            const tags = new Set()
            ;(Array.isArray(baseTags) ? baseTags : []).forEach(tag => {
                const clean = String(tag || '').trim()
                if (clean) tags.add(clean)
            })
            const pageTag = String(pageContext?.tagLabel || '').trim()
            if (pageTag) tags.add(pageTag)
            const normalizedType = String(fileType || '').replace(/^animated_/, '').trim()
            if (normalizedType) tags.add(normalizedType)
        ...
        click: async function (btn, status_id, is_exist, index) {
            ...
            try {
                const payload = await this.prepareMediaPayload(status_id, { index, promptIfQuoted: true })
                if (!payload || !payload.tasks || payload.tasks.length === 0) {
                    this.status(btn, 'failed', 'MEDIA_NOT_FOUND')
                    return
                }
                let enable_packaging = await GM_getValue('enable_packaging', true)
                this.downloader.add(payload.tasks, btn, save_history, is_exist, payload.statusId, enable_packaging)
            } catch (err) {
                if (String(err.message || err) === 'CANCELLED') {
                    this.status(btn, 'download', lang.download)
                    return
                }
                this.status(btn, 'failed', err.message || String(err))
            }
        }, downloader: (function () {
            let tasks = [], thread = 0, failed = 0, notifier, has_failed = false
            return {
                /**
                 * Chromium 系浏览器（Edge / Chrome）下载兜底：
                 * 直接 fetch 文件内容 -> Blob -> a.download。
                 *
                 * 为什么这里不用 GM_download：
                 * 1. 某些 Tampermonkey + Edge 组合下，GM_download 会显示“完成”，
                 *    但实际没有稳定落到本地下载目录；
                 * 2. 对于 X 的图片 / mp4 直链，浏览器原生下载链路更稳定；
                 * 3. 这样也更方便我们后续改造成“保存到 Eagle”。
                 */
                    ...
                    if (taskList.length > 1) {
                        ...
                        if (enable_packaging) {
                            ...
                            taskList.forEach((task) => {
                                ...
                                /**
                                 * Edge / Chrome 优先走原生下载；
                                 * Firefox 继续保留 GM_download，兼容性更好。
                                 */
                                const isFirefox = navigator.userAgent.toLowerCase().indexOf('firefox') > -1
                                if (isFirefox) {
                                    GM_download({
                        ...
                        if (isFirefox) {
                            ...
                            this.downloadViaBrowser(taskList[0])
                                .then(() => {
                                    thread--
                                    tasks = tasks.filter(t => t.url !== taskList[0].url)
                                    this.status(btn, 'completed', lang.completed)
        ...
        /**
         * Eagle 本地 API。
         * 当前先走最轻量方案：把 X 媒体的公开直链直接交给 Eagle 下载。
         */
        eagle: {
            ...
            importIndexKey: 'eagle_import_index_v1',
            importIndexCache: null,
            eagleUrlPresenceCache: {},
            // 同一条推文的多个媒体共享 website；缓存一次查询，避免逐媒体重复访问 Eagle。
            ...
            /**
             * 为每个导入任务生成稳定 key，用于本地去重。
             *
             * 这里不依赖 Eagle 侧查询，原因是：
             * 1. 本地去重速度更快；
             * 2. 不会在批量导入时频繁请求 Eagle；
             * 3. 去重键尽量不依赖“文件名”，而是优先使用稳定的媒体指纹。
             */
            ...
            buildImportUrlCandidates: function (task) {
                const rawUrl = String(task?.url || '').trim()
                const normalizedUrl = this.normalizeImportUrl(rawUrl)
                return Array.from(new Set([rawUrl, normalizedUrl].filter(Boolean)))
            },
            /**
             * Eagle 的 item.get?url=... 匹配的是素材记录中的页面来源 URL。
             * 对我们的 X 场景来说，就是 tweetUrl / website，而不是原始图片 CDN URL。
             */
            buildEagleLookupUrlCandidates: function (task) {
                const rawUrl = String(task?.website || task?.tweetUrl || '').trim()
                const normalizedUrl = this.normalizeImportUrl(rawUrl)
                const candidates = [rawUrl, normalizedUrl]
                /**
                 * 库存素材的来源 URL 存在「规范 / i/web / i」三种形态，且互相无法精确互查。
                 * 按 statusId 主动补齐可构造的变体：本次任务用规范 URL 时，
                 * 也能命中历史上以 i/web 或 i 格式入库的素材，避免 Eagle 弹重复框。
                 */
                const statusId = String(task?.statusId || '').trim()
                if (statusId) {
                    candidates.push(`https://x.com/i/web/status/${statusId}`)
                    candidates.push(`https://x.com/i/status/${statusId}`)
                }
        ...
        buildEagleDedupeTag: function (task) {
            /**
             * 这个标签会随素材一起写入 Eagle。
             * 这样即使本地脚本索引丢了，Eagle 侧也能通过这个稳定标签识别“我已经导过它”。
             */
            const statusId = String(task?.statusId || '').trim()
            ...
            const tagKey = [statusId, mediaId || mediaIndex, fileType].filter(Boolean).join(':')
            return tagKey ? `tmd:${tagKey}` : ''
        },
        buildImportKeys: function (task) {
                /**
                 * 优先级从高到低：
                 * 1. mediaFingerprint：我们自己拼出来的稳定指纹
                 * 2. statusId + url：当指纹缺失时的 fallback
                 * 3. 旧版兼容键：保留已有历史数据可继续命中
                 */
                const keys = []
                const fingerprint = String(task?.mediaFingerprint || '').trim()
                const dedupeTag = String(task?.dedupeTag || '').trim()
                const statusId = String(task?.statusId || '').trim()
                const normalizedUrl = this.normalizeImportUrl(task?.url || task?.website || task?.tweetUrl || '')
                const stableName = String(task?.name || '').trim()
                const stableWebsite = String(task?.website || task?.tweetUrl || '').trim()

                if (dedupeTag) {
                    keys.push(`v3::${dedupeTag}`)
                }
                if (fingerprint) {
                    keys.push(`v2::${fingerprint}`)
                }
                if (statusId || normalizedUrl) {
                    keys.push(`v2::${statusId}::${normalizedUrl}`)
                }
                // v1 ???????????????????????????????????
                return Array.from(new Set(keys.filter(Boolean)))
            },
            markKnownEagleUrl: function (url) {
                const normalized = this.normalizeImportUrl(url)
                if (!normalized) return
                this.eagleUrlPresenceCache[normalized] = true
            },
            /**
             * Eagle 侧二次校验：
             * - 当本地索引丢失、换设备、重装脚本时，仍然尽量避免重复导入
             * - 使用 Eagle 官方 Item API 按 source url 精确过滤
             *
             * 官方文档（Item API）说明：
             * - POST /api/v2/item/get 支持 `url` 过滤
             * - 支持 `fields` 仅返回需要的字段以提升性能
             * 来源：
            ...
            existsInEagleByUrl: async function (task) {
                ...
                for (const candidate of candidates) {
                    try {
                        ...
                        /**
                         * 这里不要因为 Eagle 校验失败就中断整个导入流程。
                         * 二次校验只是“保险”，失败时退回本地索引策略即可。
                         */
                        console.debug('[TMD][Eagle][dedupe-check]', candidate, err)
                    }
                }
                return false
            },
            ...
            hasImportedTask: async function (task) {
                ...
                const taskWebsite = String(task?.website || task?.tweetUrl || '').trim()

                if (!Array.isArray(keys) || keys.length === 0) {
                    const existsOnlyInEagle = await this.existsInEagleByUrl(task)
                    if (existsOnlyInEagle) {
                        console.debug('[TMD][Eagle][dedupe-hit][eagle-only]', { taskName, taskWebsite, taskUrl })
                    }
                    return existsOnlyInEagle
                }
                const index = await this.getImportIndex()
                const matchedLocalKey = keys.find(key => !!index[key])
                if (matchedLocalKey) {
                    console.debug('[TMD][Eagle][dedupe-hit][local-index]', { key: matchedLocalKey, taskName, taskWebsite, taskUrl })
                    return true
                }

                const existsInEagle = await this.existsInEagleByUrl(task)
                if (existsInEagle) {
                    console.debug('[TMD][Eagle][dedupe-hit][eagle-lookup]', { taskName, taskWebsite, taskUrl })
                    await this.markTaskImported(task)
                    return true
                }
                console.debug('[TMD][Eagle][dedupe-miss]', { taskName, taskWebsite, taskUrl })
                return false
            },
            /**
             * 在还没成功解析推文正文前，先按 status id / 页面来源判断是否已保存。
             * 这样新版 X 返回不完整结构时，历史已保存推文会被记为“跳过”，而不是“失败”。
             */
            hasImportedStatus: async function (statusId, urlCandidates = []) {
                const id = String(statusId || '').trim()
                if (!id) return false

                const index = await this.getImportIndex()
                const localHit = Object.keys(index || {}).some(key => {
                    return key.startsWith('v2::' + id + '::') || key.startsWith('v3::tmd:' + id + ':')
                })
                if (localHit) {
                    console.debug('[TMD][Eagle][dedupe-hit][status-index]', id)
                    return true
                }

                const candidates = Array.from(new Set((Array.isArray(urlCandidates) ? urlCandidates : [])
                    .flatMap(candidate => [candidate, this.normalizeImportUrl(candidate)])
                    .filter(Boolean)))
                for (const candidate of candidates) {
                    try {
                        const cacheKey = this.normalizeImportUrl(candidate) || candidate
                        ...
                        if (items.length > 0) {
                            console.debug('[TMD][Eagle][dedupe-hit][status-url]', { id, candidate, count: items.length })
                            return true
                        }
                    } catch (err) {
                        console.debug('[TMD][Eagle][status-dedupe-check]', candidate, err)
                    }
                }
                return false
            },
            ...
            getTagCatalog: async function (force = false) {
                ...
                // Eagle 版本差异：v2 标签接口可能 404，失败时也要回退旧接口，
                // 不能因为标签 v2 不存在而把“文件夹可读、标签不可读”误报成 Eagle 未运行。
                ...
                try {
                    ...
                    console.debug('[TMD][Eagle][tag-v2-fallback]', err)
                }
               ...
               let groupsResponse = null
                try {
                    ...
                    console.debug('[TMD][Eagle][tag-group-list]', err)
                }
            ...
            ensureFolder: async function (name, parentId = '') {
                ...
                this.pendingFolderResolvers[key] = (async () => {
                    console.info('[TMD][Eagle] create folder:', { parentId, name: safeName })
                    ...
                    /**
                     * 某些 Eagle 版本创建成功后返回体可能不完整。
                     * 这里主动刷新目录缓存，再重新查一次，尽量不要误判失败。
                     */
            ...
            /**
             * ???????????? Eagle ??????????
             * ????????????????????????????? folder / star / annotation ?????????
             */
            normalizeTask: async function (task) {
                const normalized = {
                    url: task.url,
                    name: task.name,
                    website: task.website || task.tweetUrl,
                ...
                if (itemId) {
                    for (let attempt = 1; attempt <= 8; attempt++) {
                        try {
                            ...
                            console.debug('[TMD][Eagle][folder-bind-id]', attempt, err)
                        }
                        await sleep(250)
                    }
                }
                ...
                if (lookupUrl) {
                    for (let attempt = 1; attempt <= 10; attempt++) {
                        try {
                            ...
                            console.debug('[TMD][Eagle][folder-bind-url]', attempt, err)
                        }
                        await sleep(400)
                    }
                }
            ...
            addItemByUrl: async function (task) {
                ...
                /**
                 * Eagle 部分版本会接受 item/add 请求，但没有真正写入 folders。
                 * 因此只要任务解析出了目标文件夹，就在导入后用 item/update 再确认一次。
                 * 这一步只影响新导入任务；命中去重的旧素材不会被移动。
                 */
                if (Array.isArray(normalizedTask.folders) && normalizedTask.folders.length > 0) {
                    try {
                        ...
                        console.warn('[TMD][Eagle][folder-bind-failed]', normalizedTask.name, folderErr)
                    }
                }
                await this.markTaskImported(task)
            ...
            addItemsByUrl: async function (taskList) {
                ...
                // 每轮批量任务开始时清理一次，避免长期缓存旧的 Eagle 查询结果。
                this.eagleItemLookupCache.clear()
                let savedCount = 0
                let skippedCount = 0
                const errors = []
                // Eagle 本地 API 可承受少量并发；3 个 worker 能明显缩短批量耗时，
                // 同时避免一次性并发过高导致 Eagle 队列拥塞。
                let cursor = 0
                const worker = async () => {
                    while (true) {
                        const index = cursor++
                        ...
                        try {
                            ...
                            console.error('[TMD][Eagle][add-item-failed]', taskInfo, err)
...
`
            ...
            /**
             * GraphQL 是主采集链路，页面重新可见时只补扫 DOM 首屏。
             * 该监听器不再承担“恢复后台任务”的职责，后台分页不会等待它触发。
             */
            bindVisibilityResume: function (ctx) {
                if (this.visibilityListenerBound) return
                this.visibilityListenerBound = true
                document.addEventListener('visibilitychange', () => {
                    if (!document.hidden && this.running) {
                        ctx.scanExisting(document)
                        this.setText(
                            this.mode === 'download' ? lang.batch_download_running : lang.batch_eagle_running,
                            this.formatProgress('已回到 X 页面，继续扫描时间线')
                        )
                    }
                })
            },
            isTransientTaskError: function (err) {
                const text = String(err?.message || err || '').toUpperCase()
                return /BACKGROUND_REQUEST|ABORT|TIMEOUT|NETWORK|FAILED TO FETCH|LOAD FAILED|ECONNRESET|ECONNREFUSED|X_GRAPHQL_EMPTY_RESULT|X_GRAPHQL_HTTP_4(?:00|22)|GRAPHQL_VALIDATION_FAILED|MUST BE DEFINED/.test(text)
            },
            resetTimelineState: function (ctx) {
                this.timelineCursor = ''
                this.timelineExhausted = false
                this.timelineGraphQLFailed = false
                this.timelineRestartAt = 0
                this.timelineRetryAt = 0
                this.timelineFailureCount = 0
                if (ctx) {
                    ctx.timelineQueryConfig = null
                    ctx.timelineTweetCache = new Map()
                }
            },
            /**
             * 通过 GraphQL 后台获取一页新的 status id。
             * 如果当前页面类型暂不支持或 X 返回结构变化，则交给旧的 DOM/滚动回退链路。
             */
            fetchGraphQLTimelineIds: async function (ctx) {
                if (this.timelineGraphQLFailed && Date.now() < this.timelineRetryAt) {
                    return { available: false, ids: [] }
                }
                try {
                    this.timelineGraphQLFailed = false
                    if (this.timelineExhausted) {
                        if (Date.now() < this.timelineRestartAt) return { available: true, ids: [] }
                        // 连续模式下定期从第一页重新拉取，以发现新出现的推文。
                        this.timelineCursor = ''
                        this.timelineExhausted = false
                    }
                    const page = await ctx.fetchTimelinePage(this.timelineCursor)
                    if (!(ctx.timelineTweetCache instanceof Map)) ctx.timelineTweetCache = new Map()
                    ;(Array.isArray(page.tweets) ? page.tweets : []).forEach(tweet => {
                        const id = String(tweet?.rest_id || '').trim()
                        if (id) ctx.timelineTweetCache.set(id, tweet)
                    })
                    this.timelineCursor = page.nextCursor || ''
                    this.timelineExhausted = !!page.exhausted
                    this.timelineFailureCount = 0
                    this.timelineRetryAt = 0
                    // 清掉上一轮 GraphQL 失败提示，避免请求已经恢复后界面继续显示旧错误。
                    this.lastError = ''
                    if (this.timelineExhausted) this.timelineRestartAt = Date.now() + 5000
                    return { available: true, ids: Array.isArray(page.ids) ? page.ids : [] }
                } catch (err) {
                    const errorText = String(err?.message || err || '')
                    // 422/GraphQL 校验错误通常来自 X 更换 operation id 或变量契约。
                    // 清除本轮缓存后立即重新观察页面请求，避免一直复用坏配置。
                    const shouldRefreshQuery = /X_GRAPHQL_HTTP_4(?:00|22)|GRAPHQL_VALIDATION_FAILED|MUST BE DEFINED/i.test(errorText)
                    if (shouldRefreshQuery) {
                        const operationName = ctx?.timelineQueryConfig?.operationName
                        if (operationName && this.graphQLOperationCache) delete this.graphQLOperationCache[operationName]
                        if (ctx) ctx.timelineQueryConfig = null
                        this.timelineRetryAt = Date.now() + 1000
                    }
                    this.timelineGraphQLFailed = true
                    this.timelineFailureCount++
                    this.timelineRetryAt = Date.now() + (shouldRefreshQuery
                        ? 1000
                        : Math.min(60000, 5000 * Math.pow(2, Math.min(this.timelineFailureCount - 1, 3))))
                    this.lastError = String(err?.message || err || 'X_TIMELINE_GRAPHQL_FAILED').slice(0, 160)
                    console.warn('[TMD][timeline][graphql-fallback-dom]', err)
                    return { available: false, ids: [] }
                }
            },
            ...
            getTimelineStatusIds: function (ctx) {
                const ids = []
                const seen = new Set()
                document.querySelectorAll('article').forEach(article => {
                    ...
                    const hasMedia = article.querySelector(media_selector.join(',')) || article.querySelectorAll('a[href*="/photo/"]').length > 0
                    if (!hasMedia) return
                    const statusId = ctx.getStatusId(article)
            ...
            /**
             * DOM 滚动只作为 GraphQL 不可用时的兼容回退。
             * 页面隐藏时不强行滚动，因为 X 的 DOM 懒加载依赖前台渲染；
             * 此时主循环会等待 GraphQL 的后台重试时间，而不是等待用户切回页面。
             */
            scrollForMore: async function (ctx) {
                if (document.hidden) {
                    const currentScroller = document.scrollingElement || document.documentElement || document.body
                    const snapshot = {
                        top: currentScroller ? currentScroller.scrollTop : window.scrollY,
                        height: currentScroller ? currentScroller.scrollHeight : document.body.scrollHeight,
                        articleCount: document.querySelectorAll('article').length,
                    }
                    // 后台不强行滚动 X 页面，避免懒加载未响应时循环空转。
                    return { before: snapshot, after: snapshot, changed: false, deferred: true }
                }
                const scroller = document.scrollingElement || document.documentElement || document.body
                const before = {
                    top: scroller ? scroller.scrollTop : window.scrollY,
                    height: scroller ? scroller.scrollHeight : document.body.scrollHeight,
                    articleCount: document.querySelectorAll('article').length,
                }

                /**
                 * X 的虚拟列表会回收离开视口的 article；分段滚动并在每一步扫描，
                 * 可避免一次跳到底部时漏掉中间短暂出现的 DOM 节点。
                 */
                const step = Math.max(Math.floor(window.innerHeight * 0.85), 720)
                for (let i = 0; i < 5; i++) {
                    const currentScroller = document.scrollingElement || document.documentElement || document.body
                    const currentTop = currentScroller ? currentScroller.scrollTop : window.scrollY
                ...
                const afterScroller = document.scrollingElement || document.documentElement || document.body
                const after = {
                    top: afterScroller ? afterScroller.scrollTop : window.scrollY,
                    height: afterScroller ? afterScroller.scrollHeight : document.body.scrollHeight,
                    articleCount: document.querySelectorAll('article').length,
                }
                ...
                return {
                    before,
                    after,
                    changed: after.height > before.height || after.articleCount > before.articleCount || after.top > before.top + 80,
                }
            },
            async start(ctx, mode) {
                if (this.running) return
                let localDirHandle = null
                let useLocalDirectoryWriter = false
                if (mode === 'eagle') {
                    try {
                        await ctx.eagle.checkAlive()
                        await this.refreshFolderOptions(ctx, false)
                    } catch (err) {
                        this.setText(lang.eagle_unavailable, err.message)
                        return
                    }
                }
                if (mode === 'download' && ctx.localDownload.isSupported()) {
                    try {
                        this.setText(lang.batch_download_running, lang.batch_pick_folder || '?????????????')
                        localDirHandle = await ctx.localDownload.ensureDirectory(false)
                        useLocalDirectoryWriter = !!localDirHandle
                    } catch (err) {
                        if (String(err.message || err) === 'FOLDER_PICK_CANCELLED') {
                            this.setText(lang.batch_stopped, lang.batch_folder_cancelled || '????????')
                            return
                        }
                        this.setText(lang.batch_stopped, err.message || String(err))
                        return
                    }
                }
                ...
                this.successTweets = 0
                this.failedTweets = 0
                this.successMedia = 0
                this.skippedMedia = 0
                this.failedMedia = 0
                this.lastError = ''
                this.resetTimelineState(ctx)
                this.renderButtons()
                this.bindVisibilityResume(ctx)
                this.setText(
                    mode === 'download' ? lang.batch_download_running : lang.batch_eagle_running,
                    mode === 'download' && useLocalDirectoryWriter
                        ? (lang.batch_writing_local || '????????????????...')
                        : 'GraphQL 后台分页启动中...'
                )

                let idleRounds = 0
                let lastScrollTop = -1
                // Eagle 模式到达当前末页后继续轮询首页；本地下载模式只遍历一次时间线。
                // 只有 DOM 回退链路仍沿用最多 30 轮无新增后结束的旧规则。
                const continuousMode = mode === 'eagle'

                while (this.running && (continuousMode || idleRounds < 30)) {
                    /**
                     * 每轮先补扫当前 DOM，再通过 GraphQL cursor 请求下一页。
                     * 两条来源合并去重；暂态失败的推文由 retryQueue 显式重新入队。
                     */
                    ctx.scanExisting(document)

                    // GraphQL 分页负责在后台持续发现推文；DOM 扫描作为首屏补充和接口变化回退。
                    const timelinePage = await this.fetchGraphQLTimelineIds(ctx)
                    const domIds = this.getTimelineStatusIds(ctx)
                    const ids = Array.from(new Set([...this.retryQueue, ...(timelinePage.ids || []), ...domIds]))
                    const pending = ids.filter(id => !this.processed.has(id) && !this.inFlight.has(id))

                    if (pending.length === 0) {
                        if (timelinePage.available) {
                            if (this.timelineExhausted && !continuousMode) {
                                // 一次性下载遍历到 GraphQL 末页后立即结束，不再从第一页重复扫描。
                                idleRounds = 30
                                continue
                            }
                            idleRounds = this.timelineExhausted ? idleRounds + 1 : 0
                            this.setText(
                                mode === 'download' ? lang.batch_download_running : lang.batch_eagle_running,
                                this.formatProgress(
                                    this.timelineExhausted
                                        ? 'GraphQL 已到当前末页，后台等待新推文'
                                        : 'GraphQL 后台读取下一页'
                                )
                            )
                            await ctx.sleep(this.timelineExhausted ? 5000 : 250)
                            continue
                        }

                        const scrollResult = await this.scrollForMore(ctx)
                        if (scrollResult.deferred) {
                            this.setText(
                                mode === 'download' ? lang.batch_download_running : lang.batch_eagle_running,
                                this.formatProgress('GraphQL 暂时失败，后台等待后自动重试')
                            )
                            // 页面隐藏时不依赖 DOM 滚动，也不等待用户切回标签页；
                            // 到达重试时间后继续从后台 GraphQL 分页接口恢复。
                            const retryDelay = Math.max(1000, Math.min(15000, this.timelineRetryAt - Date.now()))
                            await ctx.sleep(retryDelay)
                            continue
                        }
                        ...
                        this.setText(
                            ...
                            this.formatProgress(
                                continuousMode
                                    ? `${lang.batch_scrolling}\uff08\u6301\u7eed\u6a21\u5f0f\uff0c\u7a7a\u8f6c ${idleRounds} \u8f6e\uff0c\u70b9\u505c\u6b62\u7ed3\u675f\uff09`
                                    : `${lang.batch_scrolling}\uff08${idleRounds}/30\uff09`
                            )
                        )
                        lastScrollTop = scrollResult.after.top
                        continue
                    }

                    idleRounds = 0

                    // 本轮逐条处理前先收割 DOM 锚点：虚拟滚动会随时卸载旧推文节点，
                    // 只有推文还在视口里时记下规范链接，后续构造来源 URL 才不会退化成 i/web。
                    ctx.harvestStatusUrlHints()
                    ...
                    for (const id of pending) {
                        ...
                        this.setText(
                            mode === 'download' ? lang.batch_download_running : lang.batch_eagle_running,
                            `${lang.batch_processing} ${this.processed.size}`
                        )

                        let payload = null
                        try {
                            payload = await ctx.prepareMediaPayload(id, {
                                json: ctx.timelineTweetCache instanceof Map ? ctx.timelineTweetCache.get(id) : undefined,
                                promptIfQuoted: false,
                                preferredChoice: 'original',
                            ...
                            if (mode === 'download') {
                                for (const task of payload.tasks) {
                                    if (useLocalDirectoryWriter && localDirHandle) {
                                        await ctx.localDownload.saveTask(task, localDirHandle)
                                    } else {
                                        await ctx.downloader.downloadViaBrowser(task)
                                    }
                                    this.successMedia++
                                    await ctx.sleep(250)
                                }
                            } else {
                                const eagleResult = await ctx.eagle.addItemsByUrl(payload.tasks)
                                const savedCount = Number(eagleResult?.savedCount || 0)
                                const skippedCount = Number(eagleResult?.skippedCount || 0)
                                this.successMedia += savedCount
                                this.skippedMedia += skippedCount
                                this.setText(
                                    lang.batch_eagle_running,
                                    savedCount > 0
                                        ? `${lang.batch_processing} ${this.processed.size}\uff5c\u65b0\u589e ${savedCount}\uff5c\u8df3\u8fc7 ${skippedCount}`
                                        : `${lang.batch_processing} ${this.processed.size}\uff5c\u672c\u8f6e\u672a\u65b0\u589e\uff5c\u8df3\u8fc7 ${skippedCount}`
                                )
                            }
                            ...
                            console.error('[TMD][batch]', mode, id, err)

                            // 解析失败后再做一次 Eagle 侧确认，覆盖“旧版本已保存、
                            // 本地索引缺失或本次 X 返回结构不完整”的情况。
                            if (mode === 'eagle') {
                                try {
                                    const alreadyImported = await ctx.eagle.hasImportedStatus(id, ctx.getStatusUrlCandidates(id))
                                    if (alreadyImported) {
                                        this.skippedMedia++
                                        this.processed.add(id)
                                        this.retryQueue.delete(id)
                                    ...
                                    console.debug('[TMD][batch][post-failure-dedupe]', id, dedupeErr)
                                }
                            }

                            const retries = (this.retryCounts.get(id) || 0) + 1
                            const canRetry = this.isTransientTaskError(err) && retries <= 3
                            if (canRetry) {
                                // 网络/后台请求暂态错误不应把推文永久标记为已处理。
                                this.retryCounts.set(id, retries)
                                this.retryQueue.add(id)
                                this.lastError = `暂缓重试 ${retries}/3：${errText}`
                            } else {
                                const failedCount = payload && Array.isArray(payload.tasks) && payload.tasks.length > 0 ? payload.tasks.length : 1
                                this.processed.add(id)
                                this.retryQueue.delete(id)
                                this.retryCounts.delete(id)
                                this.failedTweets++
                                this.failedMedia += failedCount
                                this.lastError = errText
                                this.setText(
                                    mode === 'download' ? lang.batch_download_running : lang.batch_eagle_running,
                                    `\u5931\u8d25 ${this.failedTweets}\uff1a${errText}`
                                )
                            }
                        } finally {
                            // 暂态失败仍需重试时保留 GraphQL 节点，避免 cursor 已翻页后丢失解析数据。
                            if (!this.retryQueue.has(id) && ctx.timelineTweetCache instanceof Map) {
                                ctx.timelineTweetCache.delete(id)
                            }
                ...
                this.renderButtons()
                this.setText(
                    finishedByUser ? lang.batch_stopped : lang.batch_done,
                    this.formatProgress()
                )
            },
            stop: function () {
                this.running = false
                this.setText(lang.batch_stopped, this.formatProgress())
                this.renderButtons()
            }
        },
        /**
         * 页面初始化时注入批量控制面板。
         * 这里单独抽出来，便于后续如果要扩展更多批量工具时复用。
         */
        ensureBatchPanel: function () {
            this.batch.init(this)
        },
        /**
         * 在单条推文操作栏里额外插入“保存到 Eagle”按钮。
         * 当前保留原本“下载到本地”逻辑不动，两者并行存在。
         */
        addEagleButtonToActionGroup: function (btn_group, btn_share, status_id) {
            if (!btn_group || !btn_share || !status_id) return
            if (btn_group.querySelector(`.tmd-down-eagle[data-status-id="${status_id}"]`)) return

            const eagleBtn = btn_share.cloneNode(true)
            eagleBtn.dataset.statusId = status_id
            eagleBtn.classList.add('tmd-down-eagle')
            eagleBtn.title = lang.save_to_eagle

            const button = eagleBtn.querySelector('button')
            if (button) button.removeAttribute('disabled')
            const label = eagleBtn.querySelector('span')
            if (label) label.textContent = lang.save_to_eagle
            const svg = eagleBtn.querySelector('svg')
            if (svg) {
                svg.innerHTML = '<path d="M12 3v11m0 0l-4-4m4 4l4-4M4 17v1a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-1" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>'
            }

            eagleBtn.onclick = async (event) => {
                event.preventDefault()
                event.stopPropagation()
                if (eagleBtn.classList.contains('loading')) return
                this.status(eagleBtn, 'loading', lang.save_to_eagle)
                try {
                    await this.eagle.checkAlive()
                    if (await this.eagle.hasImportedStatus(status_id, this.getStatusUrlCandidates(status_id))) {
                        this.status(eagleBtn, 'completed', lang.eagle_already_saved)
                        return
                    }
                    const payload = await this.prepareMediaPayload(status_id, {
                        promptIfQuoted: true,
                        extraTags: Array.isArray(this.batch?.selectedTags) ? this.batch.selectedTags : [],
                    })
                    if (!payload || !payload.tasks || payload.tasks.length === 0) throw new Error('MEDIA_NOT_FOUND')
                    let savedCount = 0
                    let skippedCount = 0
                    for (const task of payload.tasks) {
                        const result = await this.eagle.addItemByUrl(task)
                        savedCount += Number(result?.savedCount || 0)
                        skippedCount += Number(result?.skippedCount || 0)
                    ...
                    this.status(
                        eagleBtn,
                        'completed',
                        savedCount > 0 ? lang.eagle_saved : (skippedCount > 0 ? lang.eagle_already_saved : lang.eagle_saved)
                    )
                } catch (err) {
                    this.status(eagleBtn, 'failed', err.message || String(err))
                }
            }

            const anchor = btn_share.nextSibling && btn_share.nextSibling.nextSibling ? btn_share.nextSibling.nextSibling : null
            btn_group.insertBefore(eagleBtn, anchor)
        },
        status: function (btn, css, title, style) {
            if (css) {
                btn.classList.remove('download', 'completed', 'loading', 'failed')
        ...
        settings: async function () {
            ...
            let dialog = $element(wapper, 'div', 'position: absolute; left: 50%; top: 50%; transform: translateX(-50%) translateY(-50%); width: min(720px, calc(100vw - 32px)); max-height: calc(100vh - 40px); overflow: auto; background: rgba(255,255,255,0.92); border: 1px solid rgba(15,23,42,0.10); border-radius: 16px; color: #0f172a; backdrop-filter: blur(18px); -webkit-backdrop-filter: blur(18px); box-shadow: 0 18px 48px rgba(15,23,42,0.18);')
            let title = $element(dialog, 'h3', 'margin: 0; padding: 16px 18px 10px; font-size: 16px; line-height: 1.2;', lang.dialog.title)
            let options = $element(dialog, 'div', 'margin: 0 14px 12px; padding: 8px 4px; border: 1px solid rgba(15,23,42,0.08); border-radius: 12px; background: rgba(248,250,252,0.88);')
             ...
             * - 支持单层：X
             * - 支持多层：社媒采集/X
             * 保存后无需刷新页面，后续新的导入任务会立即使用新目录规则。
             */
            let eagle_div = $element(dialog, 'div', 'margin: 0 14px 12px; padding: 12px 14px; border: 1px solid rgba(15,23,42,0.08); border-radius: 12px; background: rgba(248,250,252,0.88);')
            let eagle_root_label = $element(eagle_div, 'label', 'display: block; font-size: 13px; font-weight: 600; color: #0f172a;', lang.dialog.eagle_root || 'Eagle 根目录')
            let eagle_root_tip = $element(eagle_div, 'div', 'margin-top: 6px; font-size: 12px; line-height: 1.5; color: #475569;', lang.dialog.eagle_root_tip || '使用 / 分隔多层目录，例如：社媒采集/X')
            let eagle_root_input = $element(eagle_div, 'input', 'display: block; width: 100%; box-sizing: border-box; margin-top: 10px; padding: 10px 12px; border-radius: 10px; border: 1px solid rgba(148,163,184,0.45); background: rgba(255,255,255,0.92); color: #0f172a; outline: none;', this.getEagleRootPathText())
            let eagle_root_preview = $element(eagle_div, 'div', 'margin-top: 8px; font-size: 12px; color: #64748b;', '')
            let eagle_template_label = $element(eagle_div, 'label', 'display: block; margin-top: 14px; font-size: 13px; font-weight: 600; color: #0f172a;', lang.dialog.eagle_template || '自动子目录模板')
            let eagle_template_tip = $element(eagle_div, 'div', 'margin-top: 6px; font-size: 12px; line-height: 1.5; color: #475569;', lang.dialog.eagle_template_tip || '可用占位符：{page-scope} {page-type} {page-owner} {author}')
            let eagle_template_input = $element(eagle_div, 'input', 'display: block; width: 100%; box-sizing: border-box; margin-top: 10px; padding: 10px 12px; border-radius: 10px; border: 1px solid rgba(148,163,184,0.45); background: rgba(255,255,255,0.92); color: #0f172a; outline: none;', this.getEagleChildTemplateText())
            const updateEagleRootPreview = () => {
                const previewParts = this.parseEagleRootPath(eagle_root_input.value)
                const previewContext = {
                    folderScope: 'Laobai_Art',
                    folderLabel: '喜欢',
                    ownerHandle: 'Laobai_Art',
                }
                const previewTemplate = this.parseEagleChildTemplate(eagle_template_input.value)
                const prevTemplateState = eagle_child_template
                eagle_child_template = previewTemplate
                const childSegments = this.resolveEagleChildTemplateSegments(previewContext, 'jiojioyishujia1')
                eagle_child_template = prevTemplateState
                eagle_root_preview.textContent = `预览：${previewParts.concat(childSegments).join(' / ')}`
            }
            eagle_root_input.placeholder = 'X'
            eagle_template_input.placeholder = '{page-scope}/{page-type}'
            eagle_root_input.oninput = updateEagleRootPreview
            eagle_template_input.oninput = updateEagleRootPreview
            updateEagleRootPreview()

            let filename_div = $element(dialog, 'div', 'margin: 0 14px 14px; padding: 12px 14px; border: 1px solid rgba(15,23,42,0.08); border-radius: 12px; background: rgba(248,250,252,0.88);')
            let filename_label = $element(filename_div, 'label', 'display: block; font-size: 13px; font-weight: 600; color: #0f172a;', lang.dialog.pattern)
            let filename_input = $element(filename_label, 'textarea', 'display: block; width: 100%; box-sizing: border-box; min-height: 110px; margin-top: 10px; font-size: inherit; line-height: 1.5; padding: 10px 12px; border-radius: 10px; border: 1px solid rgba(148,163,184,0.45); background: rgba(255,255,255,0.92); color: #0f172a;', await GM_getValue('filename', filename))
...
`)
            ...
            let btn_save = $element(title, 'label', 'float: right; margin-top: -2px;', lang.dialog.save, 'tmd-btn')
            btn_save.onclick = async () => {
                const nextRootSegments = this.parseEagleRootPath(eagle_root_input.value)
                const nextChildTemplate = this.parseEagleChildTemplate(eagle_template_input.value)
                await GM_setValue('filename', filename_input.value)
                await GM_setValue('eagle_root_path', nextRootSegments.join('/'))
                await GM_setValue('eagle_child_template', nextChildTemplate)
                eagle_root_segments = nextRootSegments
                eagle_child_template = nextChildTemplate
                wapper.remove()
            }
        },
        /**
         * 动态获取 X 当前版本的 GraphQL operation id。
         *
         * X 会随前端发布更新 query id，写死旧值会导致“推文存在但解析为空”。
         * 优先复用页面已经发出的真实请求；没有观察到时再扫描当前 JS 资源；
         * 单推文接口最后保留已知 id 兜底，时间线接口则宁可回退 DOM，也不盲用旧 id。
         */
        getGraphQLOperationId: async function (operationName, fallbackId) {
            this.graphQLOperationCache = this.graphQLOperationCache || {}
            if (this.graphQLOperationCache[operationName]) return this.graphQLOperationCache[operationName]

            const urls = []
            try {
                const resources = performance.getEntriesByType('resource')
                // 当前页面已经发出过同名 GraphQL 请求时，直接复用真实 query id。
                // 这是最可靠来源，比扫描压缩后的 JS bundle 更不容易误判。
                for (let i = resources.length - 1; i >= 0; i--) {
                    const name = String(resources[i]?.name || '')
                    const match = name.match(/\/i\/api\/graphql\/([^/?]+)\/([^?]+)/)
                    if (match && decodeURIComponent(match[2]) === operationName) {
                        this.graphQLOperationCache[operationName] = match[1]
                        return match[1]
                    }
                }
                resources.forEach(entry => {
                    const name = String(entry.name || '')
                    if (/\.js(\?|$)/.test(name) && /(?:x\.com|twitter\.com|twimg\.com)/.test(name)) urls.push(name)
                })
            } catch (err) {}
            try {
                document.querySelectorAll('script[src]').forEach(script => {
                    const src = String(script.src || '')
                    if (/\.js(\?|$)/.test(src)) urls.push(src)
                })
            } catch (err) {}

            const uniqueUrls = Array.from(new Set(urls)).slice(-80).reverse()
            const patterns = [
                new RegExp(`queryId["']?\\s*[:=]\\s*["']([^"']+)["'][\\s\\S]{0,600}${operationName}`),
                new RegExp(`${operationName}[\\s\\S]{0,600}queryId["']?\\s*[:=]\\s*["']([^"']+)["']`),
                new RegExp(`operationName["']?\\s*[:=]\\s*["']${operationName}["'][\\s\\S]{0,800}(?:queryId|id)["']?\\s*[:=]\\s*["']([^"']+)["']`),
                new RegExp(`["']${operationName}["']\\s*,\\s*["']([^"']{10,})["']`),
            ]

            for (const url of uniqueUrls) {
                try {
                    const source = await this.requestTextInBackground(url, {}, 45000).catch(() => '')
                    if (!source || !source.includes(operationName)) continue
                    for (const pattern of patterns) {
                        const match = source.match(pattern)
                        if (match && match[1] && /^[A-Za-z0-9_-]{10,}$/.test(match[1])) {
                            this.graphQLOperationCache[operationName] = match[1]
                            console.debug('[TMD][GraphQL][operation-id]', operationName, match[1], url)
                            return match[1]
                        }
                    }
                } catch (err) {
                    console.debug('[TMD][GraphQL][scan-js-failed]', url, err)
                }
            }

            console.warn('[TMD][GraphQL][operation-id-fallback]', operationName, fallbackId)
            this.graphQLOperationCache[operationName] = fallbackId
            return fallbackId
        },
        fetchJson: async function (status_id) {
            // X 会随前端发布更新 query id。这里保留当前 2026-08 前端的已知 id，
            // 避免后台请求无法读取页面资源时回退到已经失效的旧 id，导致 422。
            const currentKnownOperationId = 'GZsN2Pc4knAoit6pXa4HSA'
            /*
             * 这是当前 X 前端对 TweetResultByRestId 的真实变量契约。
             * 旧 query 曾接受 with_rux_injections / withBirdwatchNotes 等参数；
             * 新 query 不再声明它们，继续传入会返回 422："must be defined"。
             */
            const variables = {
                tweetId: String(status_id),
                withCommunity: false,
            ...
            const buildUrl = id => {
                ...
                return `https://${host}/i/api/graphql/${id}/TweetResultByRestId?${params.toString()}`
            }
            const isQueryContractError = err => /X_GRAPHQL_HTTP_4(?:00|22)|GRAPHQL_VALIDATION_FAILED|MUST BE DEFINED/i.test(String(err?.message || err || ''))
            const requestByOperationId = async id => {
                const url = buildUrl(id)
                try {
                    // 批量模式优先走 Tampermonkey 扩展后台；切走标签页后仍可继续请求。
                    const data = await this.requestJsonInBackground(url, headers)
                    if (Array.isArray(data?.errors) && data.errors.length > 0) {
                        throw new Error(this.formatGraphQLPayloadErrors(data, url))
                    }
                    return data
                } catch (backgroundErr) {
                    // 查询契约错误使用同一 URL 重试没有意义；交给下一候选 query id。
                    if (isQueryContractError(backgroundErr)) throw backgroundErr
                    // 仅在扩展后台本身不可用时回退页面 fetch。
                    console.debug('[TMD][GraphQL][background-request-fallback]', backgroundErr)
                    ...
                    try {
                        ...
                        if (!result.ok) {
                            throw new Error(`X_GRAPHQL_HTTP_${result.status}: ${JSON.stringify(data).slice(0, 220)}`)
                        }
                        return data
                    } finally {
            ...
            // 当前前端的固定 query id 优先，页面缓存/动态扫描结果只作为兼容后备。
            const observedOperationId = await this.getGraphQLOperationId('TweetResultByRestId', '')
            for (const candidateId of Array.from(new Set([currentKnownOperationId, observedOperationId])).filter(Boolean)) {
                try {
                    tweet_detail = await requestByOperationId(candidateId)
                    this.graphQLOperationCache = this.graphQLOperationCache || {}
                    this.graphQLOperationCache.TweetResultByRestId = candidateId
                    break
                } catch (err) {
                    lastRequestError = err
                    if (!isQueryContractError(err)) throw err
                    if (this.graphQLOperationCache) delete this.graphQLOperationCache.TweetResultByRestId
                    console.warn('[TMD][GraphQL][retry-current-query-id]', candidateId, err)
                }
            }
            if (!tweet_detail) throw lastRequestError || new Error('X_GRAPHQL_EMPTY_RESULT')
            const rawResult = tweet_detail?.data?.tweetResult?.result
                || tweet_detail?.data?.tweetResult
                || tweet_detail?.data?.tweet_result?.result
                || tweet_detail?.data?.tweet_result
            const tweetResult = this.unwrapTweetResult(rawResult)
            if (!tweetResult) {
                const rawType = rawResult?.__typename || rawResult?.type || 'unknown'
                if (/Tombstone|Unavailable|NotFound|Suspended|Withheld/i.test(String(rawType))) {
                    throw new Error('X_TWEET_UNAVAILABLE:' + rawType)
                }
                throw new Error(`X_GRAPHQL_EMPTY_RESULT: ${JSON.stringify(tweet_detail).slice(0, 260)}`)
            }
            return tweetResult
        },
        /**
         * 根据当前 X 页面生成时间线 GraphQL 查询配置。
         * GraphQL 只负责“发现推文”；具体图片解析和 Eagle 去重仍复用原有链路。
         */
        getObservedTimelineRequest: function () {
            const pathname = String(location.pathname || '')
            const expected = (() => {
                if (/^\/home\/?$/.test(pathname)) return ['HomeTimeline', 'HomeLatestTimeline']
                if (pathname === '/i/bookmarks') return ['Bookmarks']
                if (/^\/i\/history\/likes\/?$/.test(pathname)) return ['Likes', 'UserLikes']
                if (/^\/search\/?$/.test(pathname)) return ['SearchTimeline']
                const parts = pathname.split('/').filter(Boolean)
                if (parts[1] === 'media') return ['UserMedia', 'UserPhotoTimeline']
                if (parts[1] === 'with_replies') return ['UserRepliesTimeline', 'UserTweetsAndReplies']
                if (parts.length === 1) return ['UserOriginalsTimeline', 'UserTweets']
                return []
            })()
            ...
            try {
                const resources = performance.getEntriesByType('resource')
                for (let i = resources.length - 1; i >= 0; i--) {
                    const rawUrl = String(resources[i]?.name || '')
                    const match = rawUrl.match(/\/i\/api\/graphql\/([^/?]+)\/([^?]+)/)
                    if (!match) continue
                    const operationName = decodeURIComponent(match[2])
                    if (!expected.includes(operationName)) continue
                    const parsed = new URL(rawUrl)
                    const parseJsonParam = (name, fallback) => {
                        try {
                    ...
                    return {
                        operationId: match[1],
                        operationName,
                        variables: this.resetTimelinePaginationVariables(parseJsonParam('variables', {})),
                        features: parseJsonParam('features', null),
                        fieldToggles: parseJsonParam('fieldToggles', null),
                        source: 'observed',
                    }
                }
            } catch (err) {
                console.debug('[TMD][timeline][observe-request]', err)
            }
            return null
        },
        /** 清除观察请求里属于“上一页”的游标状态，确保第一次后台请求从当前时间线首页开始。 */
        resetTimelinePaginationVariables: function (variables) {
            const clean = Object.assign({}, variables || {})
            delete clean.cursor
            delete clean.seenTweetIds
            delete clean.seen_tweet_ids
            return clean
        },
        getTimelineQueryConfig: async function () {
            const observed = this.getObservedTimelineRequest()
            if (observed) {
                this.graphQLOperationCache = this.graphQLOperationCache || {}
                this.graphQLOperationCache[observed.operationName] = observed.operationId
                return observed
            }
            const pathname = String(location.pathname || '')
            const search = new URLSearchParams(location.search || '')
            if (/^\/home\/?$/.test(pathname)) {
                // 当前前端 bundle 没有稳定的 HomeTimeline operation；没有观察到真实请求时，
                // 不再主动调用一个不存在的 fallback，交给 DOM 回退链路处理，避免无意义的 422。
                throw new Error('X_TIMELINE_OPERATION_UNOBSERVED:HomeTimeline')
            }
            if (pathname === '/i/bookmarks') {
                return { operationName: 'Bookmarks', variables: { count: 40, includePromotedContent: true } }
            }
            if (/^\/i\/history\/likes\/?$/.test(pathname)) {
                const userId = await this.resolveViewerUserId()
                if (!userId) throw new Error('X_TIMELINE_VIEWER_ID_EMPTY')
                return {
                    operationName: 'Likes',
                    operationId: 'xA8fDIbrJfy4ojjjXmSR-A',
                    variables: {
                        userId,
                        count: 40,
                        includePromotedContent: false,
                    ...
                    fieldToggles: this.buildTimelineFieldToggles(),
                }
            }
            if (pathname === '/search' || pathname === '/search/') {
                const rawQuery = search.get('q') || ''
                if (!rawQuery) throw new Error('X_TIMELINE_QUERY_EMPTY')
                return { operationName: 'SearchTimeline', operationId: 'hyPfJYJ_XAtDYoslQc-Rgg', variables: { rawQuery, count: 40, querySource: 'typed_query', product: search.get('f') === 'live' ? 'Latest' : 'Top' } }
            }

            const parts = pathname.split('/').filter(Boolean)
            const handle = parts[0] && !['i', 'explore', 'notifications', 'messages', 'settings'].includes(parts[0].toLowerCase()) ? parts[0] : ''
            if (handle && (parts[1] === 'media' || parts[1] === 'with_replies' || parts.length === 1)) {
                const userId = await this.resolveTimelineUserId(handle)
                if (!userId) throw new Error('X_TIMELINE_USER_ID_EMPTY')
                const operationName = parts[1] === 'media' ? 'UserMedia' : parts[1] === 'with_replies' ? 'UserRepliesTimeline' : 'UserTweets'
                const operationId = {
                ...
                return {
                    ...
                    variables: {
                        userId,
                        count: 40,
                        includePromotedContent: operationName === 'UserMedia' ? false : true,
                        ...(operationName === 'UserTweets' ? { withQuickPromoteEligibilityTweetFields: true } : {}),
                        withVoice: false,
                    },
                    fieldToggles: this.buildTimelineFieldToggles(),
                }
            }
            throw new Error('X_TIMELINE_UNSUPPORTED_PAGE')
        },
        /** 通过 UserByScreenName 把个人主页 handle 转成 GraphQL 所需的 userId。 */
        resolveTimelineUserId: async function (screenName) {
            const name = String(screenName || '').replace(/^@/, '').trim()
            if (!name) return ''
            this.timelineUserIdCache = this.timelineUserIdCache || {}
            if (this.timelineUserIdCache[name]) return this.timelineUserIdCache[name]
            const operationId = await this.getGraphQLOperationId('UserByScreenName', 'Gb-d6r0vxPOADdG62OEBpQ')
            if (!operationId) return ''
            // 当前 X bundle 对 UserByScreenName 的真实变量契约是 snake_case 的
            // screen_name，加上可选的 withGrokTranslatedBio；旧版的
            // withSafetyModeUserFields / withV2Timeline 等变量会直接触发 422。
            const variables = {
                screen_name: name,
                withGrokTranslatedBio: false,
            }
            const features = this.buildUserLookupFeatures()
            // UserByScreenName 当前 bundle 明确声明了 fieldToggles；漏传时 X 会返回 422。
            const fieldToggles = {
                withPayments: false,
                withAuxiliaryUserLabels: false,
            }
            ...
            params.set('features', JSON.stringify(features))
            params.set('fieldToggles', JSON.stringify(fieldToggles))
            const url = 'https://' + host + '/i/api/graphql/' + operationId + '/UserByScreenName?' + params.toString()
            const data = await this.requestJsonInBackground(url, this.buildXGraphQLHeaders())
            const result = data?.data?.user?.result || data?.data?.user_result?.result || data?.data?.user
            const userId = String(result?.rest_id || result?.id_str || '').trim()
            if (userId) this.timelineUserIdCache[name] = userId
            return userId
        },
        /** 通过当前登录会话的 Viewer 查询获取 Likes 接口要求的 userId。 */
        resolveViewerUserId: async function () {
            if (this.timelineViewerUserId) return this.timelineViewerUserId

            // X 登录 cookie 通常直接保存当前账号的数字 ID。优先本地读取，
            // 不需要额外 GraphQL 请求，也不会受 Viewer operation 更新影响。
            try {
                const rawTwid = String(this.getCookie('twid') || '')
                const decodedTwid = decodeURIComponent(rawTwid.replace(/^['"]|['"]$/g, ''))
                const cookieMatch = decodedTwid.match(/(?:^|[;&\s])u=(\d+)/)
                if (cookieMatch?.[1]) {
                    this.timelineViewerUserId = cookieMatch[1]
                    return cookieMatch[1]
                }
            } catch (err) {
                console.debug('[TMD][timeline][viewer-id-cookie]', err)
            }

            // cookie 不可读时，从 X 左侧“个人资料”入口取得当前账号 handle，
            // 再复用已校准的 UserByScreenName 查询。
            try {
                const profileLink = document.querySelector('a[data-testid="AppTabBar_Profile_Link"][href]')
                const profilePath = String(profileLink?.getAttribute('href') || '').split('?')[0]
                const profileHandle = profilePath.split('/').filter(Boolean)[0] || ''
                if (profileHandle) {
                    const profileUserId = await this.resolveTimelineUserId(profileHandle)
                    if (profileUserId) {
                        this.timelineViewerUserId = profileUserId
                        return profileUserId
                    }
                }
            } catch (err) {
                console.debug('[TMD][timeline][viewer-id-profile-link]', err)
            }

            // 最后才调用 Viewer，覆盖 cookie 被浏览器隐藏、侧栏尚未渲染的情况。
            const operationId = await this.getGraphQLOperationId('Viewer', '5XShkXk2oO2J7SYmTu6pvw')
            if (!operationId) return ''
            const variables = { withCommunitiesMemberships: false }
            const fieldToggles = {
                isDelegate: false,
                withPayments: false,
                withAuxiliaryUserLabels: false,
            ...
            const params = new URLSearchParams()
            params.set('variables', JSON.stringify(variables))
            params.set('features', JSON.stringify(this.buildViewerFeatures()))
            params.set('fieldToggles', JSON.stringify(fieldToggles))
            const url = 'https://' + host + '/i/api/graphql/' + operationId + '/Viewer?' + params.toString()
            const data = await this.requestJsonInBackground(url, this.buildXGraphQLHeaders())
            if (Array.isArray(data?.errors) && data.errors.length > 0) {
                throw new Error(this.formatGraphQLPayloadErrors(data, url, 'X_VIEWER_GRAPHQL_ERROR'))
            }
            const result = data?.data?.viewer?.user_results?.result
                || data?.data?.viewer?.user?.result
                || data?.data?.viewer?.user
                || data?.data?.viewer
            const userId = String(result?.rest_id || result?.id_str || result?.id || '').trim()
            if (userId) this.timelineViewerUserId = userId
            return userId
        },
        /** UserByScreenName 当前 operation 声明的完整 feature 集合。 */
        buildUserLookupFeatures: function () {
            return {
        ...
        /** Viewer 当前 operation 声明的完整 feature 集合。 */
        buildViewerFeatures: function () {
            return {
        ...
        buildTimelineFeatures: function () {
            return {
                articles_preview_enabled: true,
                c9s_tweet_anatomy_moderator_badge_enabled: true,
                communities_web_enable_tweet_community_results_fetch: false,
        ...
        /** 当前 X 时间线查询统一使用的 fieldToggles；缺少该参数会触发 GraphQL 422 校验错误。 */
        buildTimelineFieldToggles: function () {
            return {
                withPayments: false,
                withAuxiliaryUserLabels: false,
                withArticleRichContentState: false,
        ...
        buildXGraphQLHeaders: function () {
            const cookies = this.getCookie()
            const headers = {
                authorization: 'Bearer AAAAAAAAAAAAAAAAAAAAANRILgAAAAAAnNwIzUejRCOuH5E6I8xnZz4puTs%3D1Zv7ttfk8LF81IUq16cHjhLTvJu4FA33AGWWjCpTnA',
                'x-twitter-active-user': 'yes',
                'x-twitter-client-language': cookies.lang || 'zh-cn',
                'x-csrf-token': cookies.ct0 || '',
            }
            if (cookies.ct0 && cookies.ct0.length === 32 && cookies.gt) headers['x-guest-token'] = cookies.gt
            return headers
        },
        /** 递归提取时间线中的推文节点和下一页 cursor，兼容多种 URT 包装。 */
        extractTimelinePage: function (payload) {
            const tweets = []
            const cursors = []
            const visited = new WeakSet()
            const seenIds = new Set()
            let recognizedTimeline = false
            const visit = (node, depth = 0) => {
                if (!node || typeof node !== 'object' || depth > 22) return
                if (visited.has(node)) return
                visited.add(node)
                if (Array.isArray(node.instructions) || /^TimelineTimeline/.test(String(node.entryType || node.__typename || ''))) {
                    recognizedTimeline = true
                }
                const cursorType = String(node.cursorType || node.cursor_type || '').toLowerCase()
                if (node.value && cursorType) cursors.push({ value: String(node.value), type: cursorType })

                const candidates = [
                    node.tweet_results?.result,
                    node.tweetResults?.result,
                    node.itemContent?.tweet_results?.result,
                    node.content?.itemContent?.tweet_results?.result,
                    node.tweetResult?.result,
                ]
                for (const candidate of candidates) {
                    const tweet = this.unwrapTweetResult(candidate)
                    const id = String(tweet?.rest_id || '').trim()
                    if (tweet && id && !seenIds.has(id)) {
                        seenIds.add(id)
                        tweets.push(tweet)
                    }
                }
            ...
            visit(payload)
            const bottom = cursors.find(item => item.type === 'bottom' || item.type === 'next')
            return { tweets, nextCursor: bottom?.value || '', hasCursor: !!bottom?.value, recognizedTimeline }
        },
        /** 后台获取一页时间线推文；页面隐藏时仍可通过 GM_xmlhttpRequest 执行。 */
        fetchTimelinePage: async function (cursor = '') {
            const config = this.timelineQueryConfig || (this.timelineQueryConfig = await this.getTimelineQueryConfig())
            const operationId = config.operationId || await this.getGraphQLOperationId(config.operationName, '')
            if (!operationId) throw new Error('X_TIMELINE_OPERATION_ID_EMPTY:' + config.operationName)
            const variables = Object.assign({}, config.variables)
            if (cursor) variables.cursor = cursor
            else delete variables.cursor
            const params = new URLSearchParams()
            params.set('variables', JSON.stringify(variables))
            params.set('features', JSON.stringify(config.features || this.buildTimelineFeatures()))
            // 观察不到页面请求时也必须携带 fieldToggles；当前 X 查询会校验这组变量。
            params.set('fieldToggles', JSON.stringify(config.fieldToggles || this.buildTimelineFieldToggles()))
            const url = 'https://' + host + '/i/api/graphql/' + operationId + '/' + config.operationName + '?' + params.toString()
            let data
            try {
                data = await this.requestJsonInBackground(url, this.buildXGraphQLHeaders())
            } catch (err) {
                const message = String(err?.message || err || 'X_TIMELINE_GRAPHQL_REQUEST_FAILED')
                throw new Error(`${message} [operation=${config.operationName}; queryId=${operationId}; variables=${Object.keys(variables).join(',')}]`)
            }
            if (Array.isArray(data?.errors) && data.errors.length > 0) {
                throw new Error(this.formatGraphQLPayloadErrors(data, url, 'X_TIMELINE_GRAPHQL_ERROR [variables=' + Object.keys(variables).join(',') + ']'))
            }
            const page = this.extractTimelinePage(data)
            if (!page.recognizedTimeline && page.tweets.length === 0 && !page.hasCursor) {
                throw new Error('X_TIMELINE_UNRECOGNIZED_RESPONSE')
            }
            const mediaTweets = page.tweets.filter(tweet => {
                const legacy = tweet?.legacy || tweet
                if (this.hasDownloadableMedia(legacy)) return true
                const quoted = this.unwrapTweetResult(legacy?.quoted_status_result?.result || tweet?.quoted_status_result?.result)
                if (this.hasDownloadableMedia(quoted?.legacy || quoted)) return true
                const retweeted = this.unwrapTweetResult(legacy?.retweeted_status_result?.result || tweet?.retweeted_status_result?.result)
                return this.hasDownloadableMedia(retweeted?.legacy || retweeted)
            })
            return {
                ids: mediaTweets.map(tweet => String(tweet.rest_id)).filter(Boolean),
                tweets: mediaTweets,
                nextCursor: page.nextCursor,
                exhausted: !page.hasCursor,
            }
        },
        ...
        language: {
            en: { download: 'Download', completed: 'Download Completed', settings: 'Settings', export_history: 'Export History (Markdown)', dialog: { title: 'Download Settings', save: 'Save', save_history: 'Remember download history', clear_history: '(Clear)', clear_confirm: 'Clear download history?', show_sensitive: 'Always show sensitive content', pattern: 'File Name Pattern', eagle_root: 'Eagle Root Folder', eagle_root_tip: 'Use / to separate folder levels, for example: Social Capture/X', eagle_template: 'Auto Subfolder Template', eagle_template_tip: 'Available placeholders: {page-scope} {page-type} {page-owner} {author}' }, enable_packaging: 'Package multiple files into a ZIP', original: 'Original Tweet', quote: 'Quoted Tweet', cancel: 'Cancel', choose: 'Select media to download', save_to_eagle: 'Save to Eagle', eagle_saved: 'Saved to Eagle', eagle_already_saved: 'Already in Eagle', eagle_unavailable: 'Eagle is not running', batch_title: 'X Media Tools', batch_idle: 'Ready', batch_download: 'Batch Download', batch_download_running: 'Batch downloading timeline', batch_eagle_running: 'Saving timeline to Eagle', batch_scanning: 'Scanning current timeline...', batch_scrolling: 'Scrolling to load more...', batch_processing: 'Processing tweet', batch_progress: 'Progress', batch_done: 'Batch finished', batch_stopped: 'Batch stopped', stop: 'Stop' },
            ja: { download: 'ダウンロード', completed: 'ダウンロード完了', settings: '設定', export_history: '履歴を Markdown で書き出す', dialog: { title: 'ダウンロード設定', save: '保存', save_history: 'ダウンロード履歴を保存する', clear_history: '(クリア)', clear_confirm: 'ダウンロード履歴を削除する？', show_sensitive: 'センシティブな内容を常に表示する', pattern: 'ファイル名パターン', eagle_root: 'Eagle ルートフォルダ', eagle_root_tip: '複数階層は / で区切ります。例: Social Capture/X', eagle_template: '自動サブフォルダテンプレート', eagle_template_tip: '使用可能: {page-scope} {page-type} {page-owner} {author}' }, enable_packaging: '複数ファイルを ZIP にパッケージ化する', original: '元のツイート', quote: '引用ツイート', cancel: 'キャンセル', choose: 'メディアを選択', save_to_eagle: 'Eagle に保存', eagle_saved: 'Eagle に保存済み', eagle_already_saved: 'すでに Eagle にあります', eagle_unavailable: 'Eagle が起動していません', batch_title: 'X メディアツール', batch_idle: '準備完了', batch_download: '一括ダウンロード', batch_download_running: 'タイムラインを一括保存中', batch_eagle_running: 'Eagle に保存中', batch_scanning: 'タイムラインを解析中...', batch_scrolling: 'さらに読み込み中...', batch_processing: '処理中', batch_progress: '進捗', batch_done: '一括処理完了', batch_stopped: '一括処理停止', stop: '停止' },
            zh: { download: '下载', completed: '下载完成', settings: '设置', export_history: '导出下载记录（Markdown）', dialog: { title: '下载设置', save: '保存', save_history: '保存下载记录', clear_history: '(清除)', clear_confirm: '确认要清除下载记录？', show_sensitive: '自动显示敏感的内容', pattern: '文件名格式', eagle_root: 'Eagle 根目录', eagle_root_tip: '使用 / 分隔多层目录，例如：社媒采集/X', eagle_template: '自动子目录模板', eagle_template_tip: '可用占位符：{page-scope} {page-type} {page-owner} {author}' }, enable_packaging: '多文件打包成 ZIP', original: '原始推文', quote: '引用推文', cancel: '取消', choose: '选择要下载的媒体', save_to_eagle: '保存到 Eagle', eagle_saved: '已保存到 Eagle', eagle_already_saved: '已在 Eagle 中', eagle_unavailable: 'Eagle 未运行', batch_title: 'X 媒体工具', batch_idle: '准备就绪', batch_download: '批量下载', batch_download_running: '正在批量下载时间线', batch_eagle_running: '正在保存时间线到 Eagle', batch_scanning: '正在扫描当前时间线...', batch_scrolling: '正在向下加载更多...', batch_processing: '正在处理推文', batch_progress: '进度', batch_done: '批量任务完成', batch_stopped: '批量任务已停止', stop: '停止' },
            'zh-Hant': { download: '下載', completed: '下載完成', settings: '設置', export_history: '導出下載記錄（Markdown）', dialog: { title: '下載設置', save: '保存', save_history: '保存下載記錄', clear_history: '(清除)', clear_confirm: '確認要清除下載記錄？', show_sensitive: '自動顯示敏感的内容', pattern: '文件名規則', eagle_root: 'Eagle 根目錄', eagle_root_tip: '使用 / 分隔多層目錄，例如：社媒採集/X', eagle_template: '自動子目錄模板', eagle_template_tip: '可用佔位符：{page-scope} {page-type} {page-owner} {author}' }, enable_packaging: '多文件打包成 ZIP', original: '原始推文', quote: '引用推文', cancel: '取消', choose: '選擇要下載的媒體', save_to_eagle: '保存到 Eagle', eagle_saved: '已保存到 Eagle', eagle_already_saved: '已在 Eagle 中', eagle_unavailable: 'Eagle 未運行', batch_title: 'X 媒體工具', batch_idle: '準備就緒', batch_download: '批量下載', batch_download_running: '正在批量下載時間線', batch_eagle_running: '正在保存時間線到 Eagle', batch_scanning: '正在掃描當前時間線...', batch_scrolling: '正在向下加載更多...', batch_processing: '正在處理推文', batch_progress: '進度', batch_done: '批量任務完成', batch_stopped: '批量任務已停止', stop: '停止' }
        },
        css: `
...
.tmd-down-eagle {margin-left: 6px;}
