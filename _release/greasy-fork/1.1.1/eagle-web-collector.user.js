...
(function() {
    ...
    function getFabListingNames() {
        // og:title / twitter:title 通常保留 Fab 原始英文名，即使浏览器已经翻译了页面标题。
        const metadataTitle = document.querySelector('meta[property="og:title"]')?.getAttribute('content')
            || document.querySelector('meta[name="twitter:title"]')?.getAttribute('content')
            || '';
        const originalName = String(metadataTitle || document.title || '')
            .split('|')[0]
            .trim();
            ...
            GM_xmlhttpRequest({
                ...
                onload: async response => {
                    try {
                        ...
                        if (!translated || translated.toLowerCase() === source.toLowerCase() || !hasChineseText(translated)) {
                            resolve('');
                            return;
                        }
                        ...
                        Log.debug('解析 Fab 中文标题失败:', err?.message || err);
                        resolve('');
                    }
                },
                onerror: () => resolve(''),
    ...
    async function getFabListingNamesAsync() {
        const names = getFabListingNames();
        ...
        if (hasChineseText(names.localizedName) && names.localizedName !== source) {
            return { ...names, folderName: `${source}｜${names.localizedName}` };
        }
    ...
    const UIPanel = {
        ...
        _createStyles() {
            ...
            style.textContent = `
                 ...
                 * 但目录/标签列表恢复原先 420px 的阅读宽度，在面板外悬浮显示。
                ...
                #eagle-scraper-panel .esp-tag-search:hover {
                    border-color: rgba(255,255,255,0.24) !important;
                    background: rgba(255,255,255,0.075) !important;
                }
                #eagle-scraper-panel .esp-tag-search:focus,
                #eagle-scraper-panel .esp-tag-search:focus-visible {
                ...
                #eagle-scraper-panel .esp-tag-search::placeholder {
                ...
                <div class="esp-body">
                    ...
                    <div class="esp-switch-row esp-config-section" id="esp-auto-folder-row">
                        <label class="esp-switch-label" for="esp-auto-folder">
                            自动建目录
                            <span class="esp-help" tabindex="0" data-tooltip="Fab 英文商品名会自动补充中文译名，并按“英文｜中文”创建目录；E-Hentai 按画廊标题创建目录。" aria-label="自动建目录说明">?</span>
                        </label>
        ...
        positionPickerMenu(menu, trigger, desiredHeight) {
            ...
            const targetRight = targetLeft + menuWidth;
            menu.style.left = 'auto';
            menu.style.right = Math.round(rect.right - targetRight) + 'px';
        },
        ...
        renderEagleTagMenu() {
            ...
            const selectedContainer = this.container?.querySelector('#esp-selected-tags');
